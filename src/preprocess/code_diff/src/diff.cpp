...
void searchAndReport(
    returnParse_t* first, returnParse_t* second, returnParse_t* report,
    string exec)
{
  ...
  for (int i = 0; i < first->noOfModules; i++) {
    ...
    if (first->locateOriginals[i]
        != -1) { // duplicate - accounted for all in the original
      continue;
    }
    ...
    for (int j = 0; j < second->noOfModules; j++) {
      ...
      if (secondName.compare(firstName) == 0) {
        secondIndex
            = j; // get the first index of the match guaranteed to the original
        break;
      }
    }
    ...
    if (secondIndex != -1) {
      ...
      if (first->module[i].name.compare(second->module[secondIndex].name)
          != 0) {
        cout << "assert failed" << endl;
      }

      for (int j = 0; j < firstAdd.size(); j++) {
        if (find(secondAdd.begin(), secondAdd.end(), firstAdd[j])
            == secondAdd.end()) {
          notMatched++;
          if (printName) {
            printName = 0;
...
unsigned int
fillBBdata(returnParse_t* returnVal, const char* line, int maxModules)
{
  ...
  if (sscanf(line, "module[%u]: %llx,%u", &moduleNumber, &startAddress, &size)
      != 3) {
    cout << "assert failed" << endl;
  }
  ...
  uint64 place;
  if (find(
          returnVal->duplicateIndexes.begin(),
          returnVal->duplicateIndexes.end(), moduleNumber)
      == returnVal->duplicateIndexes.end()) { // not duplicate
    place = moduleNumber;
  } else { // duplicates
...
void removeDuplicates(returnParse_t* first)
{
  ...
  for (int i = 0; i < first->noOfModules; i++) {
    ...
    first->module[i].addresses.erase(
        unique(
            first->module[i].addresses.begin(),
...
returnParse_t* parseFiles(ifstream& file)
{
  ...
  modules_t* modules
      = new modules_t[noOfModules + 1]; // noOfModules + space for dynamically
