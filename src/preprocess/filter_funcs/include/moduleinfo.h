...
struct bbinfo_t {
  ...
  /* we are also keeping backward information - specially when global function
   * information is missing */
  uint32_t func_addr;

  std::vector<targetinfo_t*> from_bbs; /* bbs from which this bb was reached */
  std::vector<targetinfo_t*>
      to_bbs; /* to which basic blocks this bb connects to */
...
void print_funcs_filter_file(
    std::ofstream& file, std::string name, uint32_t func_addr);
