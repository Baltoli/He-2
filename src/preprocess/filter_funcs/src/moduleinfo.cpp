...
moduleinfo_t* populate_moduleinfo(ifstream& file)
{
  ...
  for (int i = 0; i < no_modules; i++) {
    ...
    for (int j = 0; j < no_bbs; j++) {
      ...
      /*func, bb_addr, size, freq, <from_bbs_count>, from_bb, freq, ..,
       * <caller_count>, caller, freq, ..., <to_bbs_count>, to_bbs, freq, ...,
       * <callee_count>, callee, freq*/
...
bbinfo_t* find_bb(funcinfo_t* func, uint32_t addr)
{
  ...
  for (int i = 0; i < func->bbs.size(); i++) {
    if ((func->bbs[i]->start_addr >= addr)
        && (addr < func->bbs[i]->start_addr + func->bbs[i]->size)) {
      return func->bbs[i];
    }
  }
  return NULL;
  ...
  for (int i = 0; i < module->funcs.size(); i++) {
    ...
    for (int j = 0; j < func->bbs.size(); j++) {
      if ((func->bbs[j]->start_addr <= addr)
          && (addr < (func->bbs[j]->start_addr + func->bbs[j]->size))) {
        return func->bbs[j];
      }
    }
  }
...
/* tries to find the occurance in the current module or else find in another
 * module addr - bb start addr
 */
bbinfo_t* find_bb(
    moduleinfo_t* head, moduleinfo_t* current, uint32_t addr,
    moduleinfo_t** from_module)
{
...
uint32_t get_probable_func_entrypoint(
    moduleinfo_t* current, queue<rec_struct>& bb_start,
    vector<uint32_t>& processed, uint32_t max_recurse)
{
  ...
  if (bbinfo == NULL) {
    return get_probable_func_entrypoint(
        current, bb_start, processed, max_recurse + 1);
  }

  if (bbinfo->is_call_target)
    ret--;
  if (ret < 0) {
    return bbinfo->start_addr;
  }
  if (bbinfo->is_ret && max_recurse > 0)
    ret++;
  ...
  for (int i = 0; i < bbinfo->from_bbs.size(); i++) {
    ...
    if (find(processed.begin(), processed.end(), bbinfo->from_bbs[i]->target)
        == processed.end()) {
      DEBUG_PRINT(("bbs - %x\n", bbinfo->from_bbs[i]->target), 20);
...
uint32_t get_probable_func(
    moduleinfo_t* head, moduleinfo_t* current, uint32_t start_addr)
{
...
void print_funcs(ofstream& file, moduleinfo_t* module)
{
  ...
  while (module != NULL) {
    ...
    for (int i = 0; i < module->funcs.size(); i++) {
      file << hex << module->funcs[i]->start_addr << "-" << dec
           << module->funcs[i]->freq << endl;
    }
    module = module->next;
  }
}
...
void print_moduleinfo(ofstream& file, moduleinfo_t* module)
{
  ...
  while (module != NULL) {
    ...
    for (int i = 0; i < module->funcs.size(); i++) {
      ...
      file << "func-" << hex << module->funcs[i]->start_addr << dec << "-"
           << module->funcs[i]->bbs.size() << "-" << module->funcs[i]->freq
           << endl;
