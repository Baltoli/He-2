...
vector<mem_info_t*>
get_function_pc_mem_regions(funcinfo_t* func, vector<pc_mem_region_t*>& regions)
{
  ...
  for (int i = 0; i < func->bbs.size(); i++) {
    ...
    for (int j = 0; j < regions.size(); j++) {
      if ((regions[j]->pc >= bbinfo->start_addr)
          && (regions[j]->pc <= bbinfo->start_addr + bbinfo->size)) {
...
void get_mem_dependant_functions(
    vector<funcinfo_t*>& funcs, funcinfo_t* src_func,
    vector<pc_mem_region_t*>& regions, vector<mem_input_t*>& mem_trace)
{
