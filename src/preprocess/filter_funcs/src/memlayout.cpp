...
vector<mem_input_t*>
get_memtrace(vector<ifstream*>& memtrace, moduleinfo_t* head)
{
  ...
  for (int i = 0; i < memtrace.size(); i++) {
    ...
    while (!(*memtrace[i]).eof()) {
      ...
      if (!line.empty()) {
        ...
        moduleinfo_t* module
            = find_module(head, strtoull(tokens[0].c_str(), NULL, 16));
        ...
        if (module != NULL) {
          ...
          DEBUG_PRINT(
              ("WARNING: cannot find a module for %s address",
               tokens[0].c_str()),
              1);
        }
      }
...
vector<mem_info_t*>
get_mem_info_from_memtrace(vector<ifstream*>& memtrace, moduleinfo_t* head)
{
...
vector<pc_mem_region_t*>
get_mem_regions_from_memtrace(vector<ifstream*>& memtrace, moduleinfo_t* head)
{
...
func_composition_t* find_func_composition(
    vector<func_composition_t*>& funcs, string module, uint32_t addr)
{
  for (int i = 0; i < funcs.size(); i++) {
    if (funcs[i]->module_name.compare(module) == 0
        && addr == funcs[i]->func_addr) {
      return funcs[i];
    }
  }
  return NULL;
...
vector<func_composition_t*> create_func_composition_func(
    vector<pc_mem_region_t*>& regions, moduleinfo_t* head)
{
  ...
  for (int i = 0; i < regions.size(); i++) {
    ...
    if (module != NULL) {
      ...
      if (func != NULL) {
        func_composition_t* comp = find_func_composition(
            funcs, regions[i]->module, func->start_addr);
        if (comp == NULL) {
...
vector<func_composition_t*> create_func_composition_wo_func(
    vector<pc_mem_region_t*>& regions, moduleinfo_t* head)
{
  ...
  for (int i = 0; i < regions.size(); i++) {
    ...
    func_composition_t* new_func
        = find_func_composition(comp, regions[i]->module, start_addr);
    if (new_func == NULL) {
...
void print_app_pc_file(ofstream& file, vector<pc_mem_region_t*>& pc_mems)
{
  ...
  for (int i = 0; i < pc_mems.size(); i++) {
    if (find(modules.begin(), modules.end(), pc_mems[i]->module)
        == modules.end()) {
