...
int main(int argc, char** argv)
{
  ...
  for (int i = 0; i < in_images.size(); i++) {
    ...
    for (int j = 0; j < files.size(); j++) {
      if (is_prefix(files[j], profile_string)) {
        ...
        ifstream* file
            = new ifstream(output_folder + "\\" + files[j], ifstream::in);
  ...
  ofstream filter_file(
      get_standard_folder("filter") + "\\" + process_name + "_" + exec + ".log",
      ofstream::out);
  ofstream app_pc_file(
      get_standard_folder("filter") + "\\" + process_name + "_" + exec
          + "_app_pc.log",
      ofstream::out);

  if (debug) {
    log_file.open(
        get_standard_folder("log") + "\\" + process_name + "_" + exec + ".log",
        ofstream::out);
  }
  ...
  5. link mem regions with consistent gaps and filter them based on the region
  size in comparison with the image sizes
  6. Rank pc_mem_regions based on size and get the pcs with the same functional
  composition as step 3
  ...
  */
  ...
  if (mode == DIFF_MODE) {
    ...
    DEBUG_PRINT(
        ("probable call stack information (upto 10 calls) for the given app_pc "
         "- %x\n",
         max_start_addr),
        1);
    for (int i = 0; i < 5; i++) {
    ...
    vector<pc_mem_region_t*> pc_mems
        = get_mem_regions_from_memtrace(memtrace_files[0], module);
    DEBUG_PRINT(("linking memory regions together... \n"), 5);
    link_mem_regions(
        pc_mems, GREEDY); /* shouldn't this return the linking information? */
    ...
    vector<mem_info_t*> total_mem_info
        = get_mem_info_from_memtrace(memtrace_files[0], module);
    ...
    /* get the pc_mems and there functional info as well as filter the pc_mems
     * which are not in the func */
    for (int i = 0; i < pc_mems.size(); i++) {
      ...
      DEBUG_PRINT(
          ("finding func start for %x in bb %x\n", pc_mems[i]->pc,
           bbinfo->start_addr),
          1);
      ...
      if (func_start == 0)
        continue;
      DEBUG_PRINT(
          ("module - %s, start - %x\n", pc_mems[i]->module.c_str(), func_start),
          1);
      bool is_there = false;
      uint32_t index = 0;
      for (int j = 0; j < func_info.size(); j++) {
        if (func_info[j]->addr == func_start
            && func_info[j]->name == md->name) {
          is_there = true;
          index = j;
          break;
        }
      ...
      if (!is_there) {
        ...
        bool found = false;
        for (int j = 0; j < func_info[index]->candidate_instructions.size();
             j++) {
    ...
    sort(
        func_info.rbegin(), func_info.rend(),
        [](internal_func_info_t* first, internal_func_info_t* second) -> bool {
          return first->freq < second->freq;
        });

    /* print out the output files - heristic we are taking the function with the
     * most amount of candidate instructions */
    print_app_pc_file(
        app_pc_file, func_info[0]->candidate_instructions, func_info[0]->name);
    print_funcs_filter_file(
        filter_file, func_info[0]->name, func_info[0]->addr);
    ...
    for (int i = 0; i < func_info.size(); i++) {
      ...
      cout << " amount of candidate instructions - " << func_info[i]->freq
           << endl;
      cout << " candidate instructions - " << endl;
      for (int j = 0; j < func_info[i]->candidate_instructions.size(); j++) {
        cout << "\t" << func_info[i]->candidate_instructions[j] << " - "
             << func_info[i]->bb_start[j] << endl;
      }
    }
