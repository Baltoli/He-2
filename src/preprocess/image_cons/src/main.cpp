...
int main(int argc, char** argv)
{
  ...
  bitmap* image = open_image((image_folder + "\\low.png").c_str());
  char* buffer = get_image_buffer(image);
  ...
  create_arith_image(
      width, height,
      (image_folder + "\\arith_" + to_string(width) + "_" + to_string(height)
       + ".png")
          .c_str());
  create_alternating_image(
      width, height, (image_folder + "\\alter.png").c_str());
...
void create_column_image(uint32_t width, uint32_t height, const char* name)
{
  bitmap* image = create_image(width, height);
  char* buffer = get_image_buffer(image);
