...
static bool parse_commandline_args(const char* args)
{
  ...
  if (dr_sscanf(
          args, "%s %d", &client_arg->filter_filename, &client_arg->filter_mode)
      != 2) {
    return false;
  }
...
dr_emit_flags_t funcreplace_bb_analysis(
...
dr_emit_flags_t funcreplace_bb_instrumentation(
...
dr_emit_flags_t funcreplace_bb_app2app(
    void* drcontext, void* tag, instrlist_t* bb, bool for_trace,
    bool translating)
{
  return DR_EMIT_DEFAULT;
...
static void pre_func_cb_2(void* wrapcxt, OUT void** user_data)
{
  ...
  amount_exec++;
  input = (uint64)drwrap_get_arg(wrapcxt, 0);
  output = (uint64)drwrap_get_arg(wrapcxt, 1);
  height = (uint64)drwrap_get_arg(wrapcxt, 2); // 120 - height
  width = (uint64)drwrap_get_arg(wrapcxt, 3); // 144 - width
  other = (uint64)drwrap_get_arg(wrapcxt, 4);
  other_2 = (uint64)drwrap_get_arg(wrapcxt, 5);

  dr_printf(
      "%d - input - %x, output - %x, %d, %d\n", amount_exec, input, output,
      width, height);
...
static void pre_func_cb(void* wrapcxt, OUT void** user_data)
{
  ...
  arg = (uint64)drwrap_get_arg(wrapcxt, 0);
  dr_printf("input - %x\n", arg);
  ...
  dr_safe_read((void*)arg, other_reg, values, &read);

  for (i = 0; i < other_reg; i++) {
    values[i] = 255 - values[i];
  }

  dr_safe_write((void*)eax, other_reg, values, &read);
...
static void code_cache_init(void)
{
  ...
  mov = INSTR_CREATE_mov_st(
      drcontext, opnd_create_reg(DR_REG_XSP), opnd_create_reg(DR_REG_XBP));
  instrlist_meta_append(ilist, mov);
  ret = INSTR_CREATE_ret(drcontext);
  instrlist_meta_append(ilist, ret);
  dr_insert_clean_call(
      drcontext, ilist, ret, (void*)clean_call_halide, false, 0);
...
void funcreplace_module_load(
    void* drcontext, module_data_t* module, bool loaded)
{
  ...
  if (md != NULL) {
    for (int i = 1; i <= md->bbs[0].start_addr; i++) {
      ...
      // module->full_path, pre_func_cb); drwrap_replace(address,
      // (app_pc)clean_call_halide, true); drwrap_replace_native(address,
      // clean_call_halide, true, 0, NULL, false);
    }
  }
}
