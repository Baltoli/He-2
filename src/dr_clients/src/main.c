...
typedef void (*init_func_t)(
    client_id_t id, const char* name, const char* arguments);
typedef void (*exit_func_t)(void);
typedef void (*module_load_t)(
...
DR_EXPORT void dr_init(client_id_t id)
{
  ...
  /*if (log_mode){
          ...
          global_logfile = dr_open_file(global_logfilename,
  DR_FILE_WRITE_OVERWRITE); DR_ASSERT(global_logfile != INVALID_FILE);
  }*/
      ...
      if (strcmp(arguments[i].name, ins_pass[j].name) == 0) {
        ...
        if (ins_pass[j].app2app_bb != NULL)
          drmgr_register_bb_app2app_event(
              ins_pass[j].app2app_bb, &ins_pass[j].priority);
        if (ins_pass[j].instrumentation_bb != NULL
            || ins_pass[j].analysis_bb != NULL)
          drmgr_register_bb_instrumentation_event(
              ins_pass[j].analysis_bb, ins_pass[j].instrumentation_bb,
              &ins_pass[j].priority);
        if (ins_pass[j].module_load != NULL)
          drmgr_register_module_load_event_ex(
              ins_pass[j].module_load, &ins_pass[j].priority);
        if (ins_pass[j].module_unload != NULL)
          drmgr_register_module_unload_event_ex(
              ins_pass[j].module_unload, &ins_pass[j].priority);
      }
    }
  }
...
// this function is responsible for setting up priorities and the order of the
...
static void setupInsPasses()
{
  ...
  drmgr_priority_t priority
      = {sizeof(priority), /* size of struct */
         ...
         DRMGR_PRIORITY_NAME_DRWRAP, /* optional name of operation we should
                                        precede */
