...
static bool parse_commandline_args(const char* args)
{
  ...
  if (dr_sscanf(
          ...
          &client_arg->filter_mode, &client_arg->app_pc_filename,
    ...
    app_pc base_pc, uint64 size, uint64 write, uint64 other_info)
{
  ...
  dr_snprintf(
      other_details, MAX_STRING_LENGTH, "%x_%d_%d_%d", base_pc, size, write,
      other_info);
  populate_conv_filename(
      filename, client_arg->output_folder, ins_pass_name, other_details);
  return filename;
}
...
static void do_mem_dump(file_t file, uint64 base_pc, uint64 size)
{
  ...
  ok = dr_safe_read((void*)(uint64)base_pc, size, mem_values, &read);
  DR_ASSERT(ok);
  written = dr_write_file(file, mem_values, size);
  DEBUG_PRINT(
      "read %d from %x of size %d and written %d\n", read, base_pc, size,
      written);
...
void clean_call_mem_information(instr_t* instr, app_pc mem_val, uint64 write)
{
  ...
  // md_add_bb_to_module(done_head, data->full_path, offset, MAX_BBS_PER_MODULE,
  // false);
  ...
  if (write) { /* postpone till the end of the function */
    if (!is_mem_region_present(
            write_regions, base_pc, size, write_region_size)) {
      DEBUG_PRINT(
          "write registered - offset - %x memval %x\n", offset, mem_val);
    ...
    if (!is_mem_region_present(read_regions, base_pc, size, read_region_size)) {
      ...
      // DEBUG_PRINT("size - %d\n", read_region_size);
      // DEBUG_PRINT("present - %d\n", is_mem_region_present(read_regions,
      // base_pc, size, read_region_size)); dr_abort();
...
dr_emit_flags_t memdump_bb_analysis(
...
dr_emit_flags_t memdump_bb_instrumentation(
    ...
    for (i = 0; i < instr_num_srcs(instr); i++) {
      if (opnd_is_memory_reference(instr_get_src(instr, i))) {
        drutil_insert_get_mem_addr(
            drcontext, bb, instr, instr_get_src(instr, i), reg1, reg2);
        dr_insert_clean_call(
            drcontext, bb, instr, clean_call_mem_information, false, 3,
            OPND_CREATE_INTPTR(instr_clones[instr_clone_amount]),
            opnd_create_reg(reg1), OPND_CREATE_INTPTR(false));
      }
    }
    for (i = 0; i < instr_num_dsts(instr); i++) {
      if (opnd_is_memory_reference(instr_get_dst(instr, i))) {
        drutil_insert_get_mem_addr(
            drcontext, bb, instr, instr_get_dst(instr, i), reg1, reg2);
        dr_insert_clean_call(
            drcontext, bb, instr, clean_call_mem_information, false, 3,
            OPND_CREATE_INTPTR(instr_clones[instr_clone_amount]),
            opnd_create_reg(reg1), OPND_CREATE_INTPTR(true));
      }
    }
...
dr_emit_flags_t memdump_bb_app2app(
    void* drcontext, void* tag, instrlist_t* bb, bool for_trace,
    bool translating)
{
  return DR_EMIT_DEFAULT;
...
static void post_func_cb(void* wrapcxt, void* user_data)
{
  ...
  for (i = 0; i < write_region_size; i++) {
    dump_filename = get_mem_dump_filename(
