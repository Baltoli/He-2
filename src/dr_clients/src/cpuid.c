...
void cpuid_init(client_id_t id, const char* name, const char* arguments)
{
...
dr_emit_flags_t cpuid_bb_app2app(
...
dr_emit_flags_t cpuid_bb_analysis(
    void* drcontext, void* tag, instrlist_t* bb, bool for_trace,
...
dr_emit_flags_t cpuid_bb_instrumentation(
    ...
    bool translating, void* user_data)
{
  ...
  */
  ...
  if (instr_get_opcode(instr) == OP_cpuid) {
    ...
    and_edx = INSTR_CREATE_and(
        drcontext, opnd_create_reg(DR_REG_EDX),
        opnd_create_immed_int(EDX_MASK, OPSZ_4));
    and_ecx = INSTR_CREATE_and(
