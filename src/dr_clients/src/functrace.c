...
void delete_function_t(void* func) { dr_global_free(func, sizeof(function_t)); }
...
#define INIT_CAPACITY 100
  ...
  if (log_mode) {
    dr_snprintf(
        thread_id, MAX_STRING_LENGTH, "%d", dr_get_thread_id(drcontext));
...
static void at_call(app_pc instr_addr, app_pc target_addr)
{
  ...
  if (module_data == NULL)
    dr_printf("instr null\n");
  if (target_module_data == NULL)
    dr_printf("target null\n");

  if (target_module_data != NULL && module_data != NULL) {

    /* call in after a jmp go back to normal tracking */
    if (!filter_from_module_name(
            head, module_data->full_path, client_arg->filter_mode)
        && filter_from_module_name(
            head, target_module_data->full_path, client_arg->filter_mode)
        && data->jmp_cnt) {
    ...
    if (filter_from_module_name(
      ...
      LOG_PRINT(
          data->logfile, "at_call_target:%s,%x\n",
          target_module_data->full_path, func->start_addr);

      if (module_data != NULL) {
        offset = (app_pc)(instr_addr - module_data->start);
        LOG_PRINT(
            data->logfile, "at_call_instr:%s,%x\n", module_data->full_path,
            offset);
      }
    }
  }
...
static void at_return(app_pc instr_addr, app_pc target_addr)
{
  ...
  if (module_data != NULL && target_module_data != NULL) {
    ...
    if (handle_jmp
      ...
      if (target_module_data != NULL) {
        offset = (app_pc)(target_addr - target_module_data->start);
        LOG_PRINT(
            data->logfile, "at_ret_target:%s,%x\n",
      ...
      offset = (app_pc)(instr_addr - module_data->start);
      LOG_PRINT(
          data->logfile, "at_ret_instr:%s,%x,%x\n", module_data->full_path,
          offset, func->start_addr);
...
static void at_cti(app_pc instr_addr, app_pc target_addr)
{
  ...
  if (module_data != NULL && target_module_data != NULL) {
    ...
    if (filter_from_module_name(
      ...
      LOG_PRINT(
          data->logfile, "at_cti_instr:%s,%x\n", module_data->full_path,
          offset);
      offset = (app_pc)(target_addr - target_module_data->start);
      LOG_PRINT(
          data->logfile, "at_cti_target:%s,%x\n", target_module_data->full_path,
          offset);
...
dr_emit_flags_t functrace_bb_app2app(
...
dr_emit_flags_t functrace_bb_analysis(
    void* drcontext, void* tag, instrlist_t* bb, bool for_trace,
...
dr_emit_flags_t functrace_bb_instrumentation(
  ...
  if (instr_ok_to_mangle(instr)) {
    ...
    if (instr_is_call_direct(instr)) {
      ...
      dr_insert_mbr_instrumentation(
          drcontext, bb, instr, (app_pc)at_call, SPILL_SLOT_1);
    } else if (instr_is_return(instr)) {
      dr_insert_mbr_instrumentation(
          drcontext, bb, instr, (app_pc)at_return, SPILL_SLOT_1);
    } else if (instr_is_mbr(instr)) {
      dr_insert_mbr_instrumentation(
          drcontext, bb, instr, (app_pc)at_cti, SPILL_SLOT_1);
