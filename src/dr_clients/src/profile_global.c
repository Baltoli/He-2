...
TODO:
 ...
 1.track all the bbs, but report only the reported basic blocks (track means
keep per thread information for all the bbs,but only upate module information
for the ones you want) 2.intra module call addresses and bb jumps are not
handled and are erroneous 3.to_bbs and calls_to information 4.information is
collected in a global structure, but we could move on to a per thread structure
-> do it in another implementation 5.Move to module->function->bb structure
...
/*
        ...
        3. If bb is a call target who called it + frequencies + bb which called
   it
...
/*
   ...
   then the next bb (the first instruction) will be the call target - record
   this
...
static bool parse_commandline_args(const char* args)
{
  ...
  if (dr_sscanf(
          args, "%s %d %s %s", &client_arg->filter_filename,
...
void bbinfo_init(client_id_t id, const char* name, const char* arguments)
{
  file_t in_file;
  ...
  populate_conv_filename(
      filename, client_arg->output_folder, name, client_arg->extra_info);
  ...
  // string pointers
  string_pointers
      = (char**)dr_global_alloc(sizeof(char*) * MAX_STRING_POINTERS);
...
void bbinfo_thread_init(void* drcontext)
{
  ...
  per_thread_data_t* data = (per_thread_data_t*)dr_thread_alloc(
      drcontext, sizeof(per_thread_data_t));
...
void bbinfo_thread_exit(void* drcontext)
{
  ...
  per_thread_data_t* data
      = (per_thread_data_t*)drmgr_get_tls_field(drcontext, tls_index);
...
static void populate_call_target_information()
{
  ...
  while (local_head != NULL) {
    for (i = 1; i <= local_head->bbs[0].start_addr; i++) {
      bb = md_lookup_bb_in_module(
          call_target_head, local_head->module, local_head->bbs[i].start_addr);
      if (bb != NULL) {
...
static void print_readable_output()
{
  ...
  while (local_head != NULL) {
    ...
    for (i = 1; i <= size; i++) {
      dr_fprintf(
          out_file, "%x - %u - ", local_head->bbs[i].start_addr,
          local_head->bbs[i].freq);

      for (j = 1; j <= local_head->bbs[i].from_bbs[0].start_addr; j++) {
        dr_fprintf(
            out_file, "%x(%u) ", local_head->bbs[i].from_bbs[j].start_addr,
            local_head->bbs[i].from_bbs[j].freq);
      }
      ...
      for (j = 1; j <= local_head->bbs[i].called_from[0].bb_addr; j++) {
        dr_fprintf(
            out_file, "%x - %x(%u) ", local_head->bbs[i].called_from[j].bb_addr,
...
/*
still we have not implemented inter module calls/bb jumps; we only update bb
    ...
    void* bb, int offset, const char* module, uint is_call, uint call_addr)
{
  ...
  if (!have_bb && (bbinfo->from_bbs[0].start_addr < MAX_TARGETS - 1)) {
    bbinfo->from_bbs[++(bbinfo->from_bbs[0].start_addr)].start_addr
        = data->prev_bb_start_addr;
  ...
  if (data->is_call_ins) {
    ...
    if (!have_call && (bbinfo->called_from[0].bb_addr < MAX_TARGETS - 1)) {
      bbinfo->called_from[++(bbinfo->called_from[0].bb_addr)].bb_addr
          = data->prev_bb_start_addr;
      bbinfo->called_from[(bbinfo->called_from[0].bb_addr)].call_point_addr
          = data->call_ins_addr;
...
dr_emit_flags_t bbinfo_bb_app2app(
...
dr_emit_flags_t bbinfo_bb_analysis(
    void* drcontext, void* tag, instrlist_t* bb, bool for_trace,
    ...
    if (strcmp(module->full_path, target_module->full_path) == 0) {
      ...
      if (!found) {
        ...
        if (data->bbinfo->called_to[0].bb_addr < MAX_TARGETS - 1) {
          ...
          data->bbinfo->called_to[data->bbinfo->called_to[0].bb_addr].bb_addr
              = offset;
          offset = instr_addr - module->start;
          data->bbinfo->called_to[data->bbinfo->called_to[0].bb_addr]
              .call_point_addr
              = offset;
...
static void call_target_info(app_pc instr_addr, app_pc target_addr)
{
  ...
  if (module_data != NULL) {
    ...
    dr_mutex_lock(stats_mutex);
    bb = md_lookup_bb_in_module(
        call_target_head, module_data->full_path, offset);
    if (bb == NULL) {
      md_add_bb_to_module(
          call_target_head, module_data->full_path, offset, MAX_BBS_PER_MODULE,
          false);
    }
...
dr_emit_flags_t bbinfo_bb_instrumentation(
    void* drcontext, void* tag, instrlist_t* bb, instr_t* instr_current,
    bool for_trace, bool translating, void* user_data)
{
  ...
  // get the module data and if module + addr is present then add frequency
  // counting
  ...
  offset = (int)(instr_get_app_pc(first) - module_data->start);
  ...
  if (client_arg->filter_mode == FILTER_MODULE) {
    if (filter_module_level_from_list(filter_head, first)) {
      ...
      if (bbinfo == NULL) {
        bbinfo = md_add_bb_to_module(
            info_head, module_data->full_path, offset, MAX_BBS_PER_MODULE,
            true);
      }
    ...
    if (bbinfo == NULL) {
      bbinfo = md_add_bb_to_module(
          info_head, module_data->full_path, offset, MAX_BBS_PER_MODULE, true);
    }
    DR_ASSERT(bbinfo != NULL);
  } else if (client_arg->filter_mode == FILTER_NEG_MODULE) {
    if (filter_from_module_name(
            filter_head, module_name, client_arg->filter_mode)) {
      if (bbinfo == NULL) {
        bbinfo = md_add_bb_to_module(
            info_head, module_data->full_path, offset, MAX_BBS_PER_MODULE,
            true);
  ...
  /* if the instr is filtered; only if instr == first; as this will be in the
   * diff file */
  if (filtered) {
    ...
    md_lookup_module(info_head, module_name)->start_addr
        = (uint64)module_data->start;

    // check whether this bb has a call at the end or a ret at the end
    instr = instrlist_last(bb);
    is_call = instr_is_call(instr);
    if (is_call) {
      call_addr = (int)(instr_get_app_pc(instr) - module_data->start);
    }
    is_ret = instr_is_return(instr);

    bbinfo->is_call = is_call;
    bbinfo->is_ret = is_ret;
    bbinfo->size = instr_get_app_pc(instrlist_last(bb))
                   - instr_get_app_pc(first)
                   + instr_length(drcontext, instrlist_last(bb));
    ...
    dr_insert_clean_call(
        drcontext, bb, first, (void*)bbinfo_population, false, 5,
        OPND_CREATE_INTPTR(bbinfo), OPND_CREATE_INT32(offset),
        OPND_CREATE_INTPTR(module_name), OPND_CREATE_INT32(is_call),
  ...
  if (!filtered) {
    ...
    if (instr_is_call_direct(last)) {
      ...
      call_target_info_wo_called_to(
          instr_get_app_pc(last), opnd_get_addr(instr_get_src(last, 0)));
    } else if (instr_is_call_indirect(last)) {
      dr_insert_mbr_instrumentation(
          drcontext, bb, last, (app_pc)call_target_info_wo_called_to,
          SPILL_SLOT_1);
    }

  } else {
    if (instr_current == last) {
      // dr_insert_clean_call(drcontext, bb, instr_current, (void *)register_bb,
      // false, 1, OPND_CREATE_INTPTR(bbinfo));
      if (instr_is_call_direct(last)) {
        dr_insert_call_instrumentation(
            drcontext, bb, instr_current, (app_pc)call_target_info);
      } else if (instr_is_call_indirect(last)) {
        dr_insert_mbr_instrumentation(
            drcontext, bb, instr_current, (app_pc)call_target_info,
            SPILL_SLOT_1);
