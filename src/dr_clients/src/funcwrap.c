...
dr_emit_flags_t funcwrap_bb_instrumentation(
    ...
    bool translating, void* user_data)
{
  ...
  if (module_data != NULL) {
    ...
    if (md != NULL) {
      offset = (unsigned char*)(pc - module_data->start);

      for (int i = 1; i <= md->bbs[0].start_addr; i++) {
        if ((uint64)offset == md->bbs[i].start_addr) {
          DEBUG_PRINT("bb instrumenting function\n");
          data->filter_func = true;
          dr_insert_clean_call(
              drcontext, bb, instr, clean_call, false, 1,
...
void funcwrap_module_load(void* drcontext, module_data_t* module, bool loaded)
{
  ...
  if (md != NULL) {
    for (int i = 1; i <= md->bbs[0].start_addr; i++) {
      ...
      DEBUG_PRINT(
          "funcwrap: %s module %x function wrapping\n", md->module, address);
