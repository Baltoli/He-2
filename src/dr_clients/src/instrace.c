...
// debug prints
//#define DEBUG_MEM_REGS   /* prints out the memory regs before dr util mem
// address calculation */ #define DEBUG_MEM_STATS  /* prints out the memory
// values - stats at that given pc */
...
static void dynamic_info_instrumentation(
    void* drcontext, instrlist_t* ilist, instr_t* where, instr_t* static_info);
...
static bool parse_commandline_args(const char* args)
{
  ...
  if (dr_sscanf(
          args, "%s %d %s %d %d %s", &client_arg->filter_filename,
          &client_arg->filter_mode, &client_arg->output_folder,
          &client_arg->static_info_size, &client_arg->instrace_mode,
...
void instrace_thread_init(void* drcontext)
{
  ...
  if (log_mode) {
    ...
    data->logfile = dr_open_file(
        logfilename, DR_FILE_WRITE_OVERWRITE | DR_FILE_ALLOW_LARGE);
  }
  ...
  dr_snprintf(
      extra_info, MAX_STRING_LENGTH, "%s_%s_%s", client_arg->extra_info, mode,
      thread_id);

  populate_conv_filename(
      outfilename, client_arg->output_folder, ins_pass_name, extra_info);
  data->outfile = dr_open_file(
      outfilename, DR_FILE_WRITE_OVERWRITE | DR_FILE_ALLOW_LARGE);
  DR_ASSERT(data->outfile != INVALID_FILE);

  DEBUG_PRINT(
      "%s - thread id : %d, new thread logging at - %s\n", ins_pass_name,
      dr_get_thread_id(drcontext), logfilename);

  data->static_array = (instr_t**)dr_thread_alloc(
      drcontext, sizeof(instr_t*) * client_arg->static_info_size);
  ...
  DEBUG_PRINT(
      "%s - thread %d stack information - stack_base %x stack_reserve %x\n",
      ins_pass_name, dr_get_thread_id(drcontext), data->stack_base,
      data->deallocation_stack);
...
void instrace_thread_exit(void* drcontext)
{
  ...
  DEBUG_PRINT(
      "%s - thread id : %d, cloned instructions freeing now - %d\n",
  ...
  dr_thread_free(
      drcontext, data->static_array,
      sizeof(instr_t*) * client_arg->static_info_size);
  dr_thread_free(drcontext, data, sizeof(per_thread_t));

  DEBUG_PRINT(
      "%s - exiting thread done %d\n", ins_pass_name,
      dr_get_thread_id(drcontext));
}
 ...
 */
dr_emit_flags_t instrace_bb_app2app(
 ...
 */
dr_emit_flags_t instrace_bb_analysis(
    void* drcontext, void* tag, instrlist_t* bb, bool for_trace,
...
/* event_bb_insert calls necessary functions to fill up the static info about an
   instruction and to instrument memory instructions to get its runtime address
   using dynamic instrumentation
 */
dr_emit_flags_t instrace_bb_instrumentation(
    ...
    bool translating, void* user_data)
{
  /* algorithm
         ...
         2. call the static info filler function to get a slot at the global
     instruction array
  ...
  if (filter_from_list(head, instr, client_arg->filter_mode)
      && should_filter_thread(dr_get_thread_id(drcontext))) {
    ...
    if (instr_info != NULL) {
      ...
      DR_ASSERT(
          client_arg->instrace_mode == INS_TRACE
          || client_arg->instrace_mode == DISASSEMBLY_TRACE);
...
static instr_t* static_info_instrumentation(void* drcontext, instr_t* instr)
{
  /*
          ...
          this should return canonicalized static info about an instruction;
     breaking down any complex instructions if necessary

          1) check whether this instruction needs to be instrumented
          2) if yes, then get a location and then proceed to instrument ->
     return the struct 3) if no, return null
  ...
  */
  ...
  if ((client_arg->instrace_mode == OPERAND_TRACE)
      || (client_arg->instrace_mode == INS_DISASM_TRACE)) {
    operand_trace(instr, drcontext);
    return NULL;
  }

  /* check whether this instr needs instrumentation - check for ones to skip and
   * skip if */
  /*switch(opcode){
  case OP_jecxz:
          return NULL;
    ...
    void* drcontext, instrlist_t* ilist, instr_t* where, instr_t* static_info)
{
  ...
  */
  ...
  /*
          ...
          we get a filled pointer here about the operand types for a given
     instruction (srcs and dests) 1) increment the pointer to the instr_trace
     buffers 2) add this pointer to instr_trace_t wrapper 3) check whether any
     of the srcs and dests have memory operations; if so add a lea instruction
     and get the dynamic address Add this address to instr_trace_t structure 4)
     if the buffer is full call a function to dump it to the file and restore
     the head ptr of the buffer (lean function is used utilizing a code cache to
     limit code bloat needed for a clean call before every instruction.)
  */
  ...
  if (client_arg->instrace_mode == DISASSEMBLY_TRACE) {
    dr_insert_clean_call(
        drcontext, ilist, where, clean_call_disassembly_trace, false, 0);
    return;
  }
  ...
  /* Move static_info to static_info_instr field of buf (which is a
   * instr_trace_t *) */
  opnd1 = OPND_CREATE_MEMPTR(reg2, offsetof(instr_trace_t, static_info_instr));
  instrlist_insert_mov_immed_ptrsz(
      drcontext, (ptr_int_t)static_info, opnd1, ilist, where, &first, &second);

  /* buf_ptr->num_mem = 0; */
  opnd1 = OPND_CREATE_MEMPTR(reg2, offsetof(instr_trace_t, num_mem));

  instrlist_insert_mov_immed_ptrsz(
      drcontext, (ptr_int_t)0, opnd1, ilist, where, &first, &second);
  ...
  for (i = 0; i < instr_num_dsts(where); i++) {
    if (opnd_is_memory_reference(instr_get_dst(where, i))) {
      ...
      dr_insert_clean_call(
          drcontext, ilist, where, clean_call_disassembly_trace, false, 0);
      dr_insert_clean_call(
          drcontext, ilist, where, clean_call_print_regvalues, false, 0);
#endif
      ...
      dr_insert_clean_call(
          drcontext, ilist, where, clean_call_disassembly_trace, false, 0);
      dr_insert_clean_call(
          drcontext, ilist, where, clean_call_mem_stats, false, 1,
          opnd_create_reg(reg1));
#endif

      dr_insert_clean_call(
          drcontext, ilist, where, clean_call_populate_mem, false, 3,
          opnd_create_reg(reg1), OPND_CREATE_INT32(i),
          OPND_CREATE_INT32(DST_TYPE));
    }
  }
  ...
  // dynamically generated code - module information not available - then just
  // store 0 at the pc slot of the instr_trace data
  if (module_data != NULL) {
  ...
  instrlist_insert_mov_immed_ptrsz(
      drcontext, (ptr_int_t)pc, opnd1, ilist, where, &first, &second);

  /* buf_ptr++; */
  /* Increment reg value by pointer size using lea instr */
  opnd1 = opnd_create_reg(reg2);
  opnd2 = opnd_create_base_disp(
      reg2, DR_REG_NULL, 0, sizeof(instr_trace_t), OPSZ_lea);
  ...
  // instrlist_disassemble(drcontext, instr_get_app_pc(instrlist_first(ilist)),
  // ilist, logfile);
}
...
static void print_base_disp_for_lea(file_t file, opnd_t opnd)
{
  ...
  dr_fprintf(
      file, " base - %d %s\n", opnd_get_base(opnd),
      get_register_name(opnd_get_base(opnd)));
  dr_fprintf(
      file, " index - %d %s\n", opnd_get_index(opnd),
      get_register_name(opnd_get_index(opnd)));
  dr_fprintf(
...
/* this is only called when the instrace mode is operand trace (this happens at
  ...
  if (client_arg->instrace_mode == OPERAND_TRACE) {
    ...
    for (i = 0; i < instr_num_dsts(instr); i++) {
      opnd_disassemble_to_buffer(
          drcontext, instr_get_dst(instr, i), stringop, MAX_STRING_LENGTH);
      if ((instr_get_opcode(instr) == OP_lea)
          && opnd_is_base_disp(instr_get_dst(instr, i))) {
    ...
    for (i = 0; i < instr_num_srcs(instr); i++) {
      opnd_disassemble_to_buffer(
          drcontext, instr_get_src(instr, i), stringop, MAX_STRING_LENGTH);
      if ((instr_get_opcode(instr) == OP_lea)
          && opnd_is_base_disp(instr_get_src(instr, i))) {
    ...
    if (module_data != NULL) {
      if (md_get_module_position(instrace_head, module_data->full_path) == -1) {
        md_add_module(
            instrace_head, module_data->full_path, MAX_BBS_PER_MODULE);
      }
      dr_fprintf(
          data->outfile, "%d,%d_%s_%s\n",
          md_get_module_position(instrace_head, module_data->full_path), pc,
          stringop, module_data->full_path);
    } else {
...
/* this is only called when the instrace mode is disassembly trace (this happens
...
static void clean_call_disassembly_trace()
{
  ...
  instr_disassemble_to_buffer(
      dr_get_current_drcontext(), trace->static_info_instr, disassembly,
      SHORT_STRING_LENGTH);
  ...
  if (md != NULL) {
    dr_fprintf(
        data->outfile, "%x",
        instr_get_app_pc(trace->static_info_instr) - md->start);
...
static void clean_call_populate_mem(reg_t regvalue, uint64 pos, uint64 dest_or_src)
{
  ...
  /* assuming the thread init gives out stack bounds properly we can select the
  ...
  if (opnd_is_reg(opnd)) {
    ...
    if (opnd_is_immed_float(opnd)) {
      ...
      if (instr_get_opcode(instr) == OP_fld1) {
        ...
        DISPLAY_STRING("immediate float unknown\n");
        dr_abort();
      }
...
static uint64 calculate_operands(instr_t* instr, uint64 dst_or_src)
{
  ...
  if (dst_or_src == DST_TYPE) {
    for (i = 0; i < instr_num_dsts(instr); i++) {
      ...
      if (opnd_is_immed(op) || opnd_is_memory_reference(op)
          || opnd_is_reg(op)) {
        ret++;
      }
    }
    ...
    for (i = 0; i < instr_num_srcs(instr); i++) {
      ...
      if (instr_get_opcode(instr) == OP_lea && opnd_is_base_disp(op)) {
        ret += 4;
          ...
          opnd_is_immed(op) || opnd_is_memory_reference(op)
          || opnd_is_reg(op)) {
        ret++;
      }
    }
...
static void ins_trace(void* drcontext)
{
  ...
  for (i = 0; i < num_refs; i++) {
    ...
    instr_disassemble_to_buffer(
        drcontext, instr, disassembly, SHORT_STRING_LENGTH);
    ...
    for (j = 0; j < instr_num_dsts(instr); j++) {
      ...
      output_populator_printer(
          drcontext, instr_get_dst(instr, j), instr, mem_addr, mem_type, NULL);
      opnd = instr_get_dst(instr, j);
      if (opnd_is_memory_reference(opnd)) {
        DR_ASSERT(opnd_is_base_disp(opnd) || opnd_is_abs_addr(opnd));
        output_populator_printer(
            drcontext, opnd_create_reg(opnd_get_base(opnd)), instr, mem_addr,
            mem_type, NULL);
        output_populator_printer(
            drcontext, opnd_create_reg(opnd_get_index(opnd)), instr, mem_addr,
            mem_type, NULL);
        output_populator_printer(
            drcontext, opnd_create_immed_int(opnd_get_scale(opnd), OPSZ_PTR),
            instr, mem_addr, mem_type, NULL);
        output_populator_printer(
            drcontext, opnd_create_immed_int(opnd_get_disp(opnd), OPSZ_PTR),
            instr, mem_addr, mem_type, NULL);
      }
    }
    ...
    for (j = 0; j < instr_num_srcs(instr); j++) {
      ...
      if (instr_get_opcode(instr) == OP_lea && opnd_is_base_disp(opnd)) {
        ...
        DR_ASSERT(opnd_is_base_disp(opnd) || opnd_is_abs_addr(opnd));
        output_populator_printer(
            drcontext, opnd, instr, mem_addr, mem_type, NULL);
        output_populator_printer(
            drcontext, opnd_create_reg(opnd_get_base(opnd)), instr, mem_addr,
            mem_type, NULL);
        output_populator_printer(
  ...
  for (i = 0; i < num_refs; i++) {
    ...
    for (j = 0; j < instr_num_dsts(instr); j++) {
      ...
      output_populator_printer(
          drcontext, instr_get_dst(instr, j), instr,
          get_address(instr_trace, j, DST_TYPE),
          &output->dsts[output->num_dsts]);
      output->num_dsts++;
    }

    for (j = 0; j < instr_num_srcs(instr); j++) {
      output_populator_printer(
          drcontext, instr_get_src(instr, j), instr,
          get_address(instr_trace, j, SRC_TYPE),
          &output->srcs[output->num_srcs]);
      output->num_srcs++;
    }
...
static void code_cache_init(void)
{
  ...
  /* clean call */
  dr_insert_clean_call(
      drcontext, ilist, where, (void*)clean_call_ins_trace, false, 0);
  /* Encodes the instructions into memory and then cleans up. */
  end = instrlist_encode(drcontext, ilist, code_cache, false);
  DR_ASSERT((end - code_cache) < sysconf(_SC_PAGE_SIZE));
  instrlist_clear_and_destroy(drcontext, ilist);
  /* set the memory as just +rx now */
  dr_memory_protect(
      code_cache, sysconf(_SC_PAGE_SIZE), DR_MEMPROT_READ | DR_MEMPROT_EXEC);
}

static void code_cache_exit(void)
{
  dr_nonheap_free(code_cache, sysconf(_SC_PAGE_SIZE));
}
...
static void clean_call_mem_stats(reg_t memvalue)
{
  ...
  dr_query_memory_ex((byte*)memvalue, &info);

  dr_printf(
      "mem - %d, base_pc - %d , size - %d, prot - %d, type - %d\n", memvalue,
      info.base_pc, info.size, info.prot, info.type);
