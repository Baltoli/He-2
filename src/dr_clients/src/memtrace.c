...
static void instrument_mem(
    void* drcontext, instrlist_t* ilist, instr_t* where, int pos, bool write);
...
void memtrace_thread_init(void* drcontext)
{
  ...
  dr_snprintf(
      extra_info, MAX_STRING_LENGTH, "%s_%s", client_arg->extra_info,
      thread_id);
  ...
  // Unclear why we need this - comment above suggests that it's somehow
  // related to 32-bit support.
  /* __asm { */
  /* mov EAX, FS : [0x04] */
  /* mov EBX, stack_base */
  /* mov[EBX], EAX */
  ...
  DEBUG_PRINT(
      "%s - stack boundaries - %x,%x\n", ins_pass_name, data->stack_base,
      data->stack_limit);
 ...
 */
dr_emit_flags_t memtrace_bb_app2app(
 ...
 */
dr_emit_flags_t memtrace_bb_analysis(
    void* drcontext, void* tag, instrlist_t* bb, bool for_trace,
...
bool should_memory_be_instrumented(opnd_t opnd)
{
  ...
  if (reg != 0 && reg != DR_REG_XBP && reg != DR_REG_XSP)
    return true;
 ...
 */
dr_emit_flags_t memtrace_bb_instrumentation(
    ...
    bool translating, void* user_data)
{
  ...
  if (instr_ok_to_mangle(instr)) {
    ...
    for (current = instrlist_first(bb); current != NULL;
         current = instr_get_next(current)) {
    ...
    if ((first != NULL)
        && filter_from_list(head, first, client_arg->filter_mode)) {
...
#ifdef READABLE_TRACE
  /*dr_fprintf(data->log,
             "Format: <instr address>,<(r)ead/(w)rite>,<data size>,<data
     address>\n");*/
  ...
  for (i = 0; i < num_refs; i++) {
    ...
    if (mdata != NULL) {
      ...
      dr_fprintf(
          ...
          mem_ref->pc - mdata->start, mem_ref->write ? 1 : 0, mem_ref->size,
          mem_ref->addr);
      //}
    }
...
#else
  dr_write_file(
      data->log, data->buf_base, (size_t)(data->buf_ptr - data->buf_base));
#endif
    ...
    void* drcontext, instrlist_t* ilist, instr_t* where, int pos, bool write)
{
  ...
  opnd2 = opnd_create_base_disp(
      reg2, DR_REG_NULL, 0, sizeof(mem_ref_t), OPSZ_lea);
