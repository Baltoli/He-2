...
bool filter_from_list(module_t * head, instr_t * instr, uint64 mode); /* can be used for clients who do not need to do extra processing after filter for each differently */
...
uint64 populate_conv_filename(char * dest,const char * folder,const char * name, const char * other_details);
