...
/* should we record the smallest stride? or the most frequent stride (width) as
 * the data element width
 */
...
static bool
compare_pc_mem_regions(pc_mem_region_t* first, pc_mem_region_t* second)
{
  ...
  return highest_first
         >= highest_second; /* we are going to get the reversed ranking */
}
...
static void update_stride_from_vector(
    vector<pair<uint, uint>>& strides, vector<pair<uint, uint>>& old)
{
...
void update_mem_regions(vector<mem_info_t*>& mem_info, mem_input_t* input)
{
  ...
  if (!merged) { /* if not merged to an exising mem_region then we need to
                    create a new region */
...
static void defragment_regions(vector<mem_info_t*>& mem_info)
{
  ...
  /*this will try to merge individual chunks of memory units info defragmented
   * larger chunks*/
  bool finished = false;
    ...
    for (int i = 0; i < mem_info.size(); i++) {
      ...
      for (int j = i + 1; j < mem_info.size(); j++) {
        ...
        if (current->type == candidate->type) {
          ...
          /*can we merge the two? we will always update the candidate and delete
           * the current if this can be merged*/
          /* we cannot possibly have a mem region captured within a mem region
           * if update mem regions has done its job*/
          ...
          if ((candidate->start >= current->start)
            ...
            update_stride_from_vector(
                current->stride_freqs, candidate->stride_freqs);
            delete mem_info[i];
            mem_info.erase(mem_info.begin() + i--);
            break;
          }
          ...
          if (merged) {
            ...
            update_stride_from_vector(
                candidate->stride_freqs, current->stride_freqs);
            delete mem_info[j];
...
static pc_mem_region_t* get_pc_mem_region(
    vector<pc_mem_region_t*>& pc_mems, string module, uint32_t app_pc)
{

  /* first check whether we have the same app_pc */
  if (!module.empty()) { /* here the module information comparison is requested
                            by the input */
    for (int i = 0; i < pc_mems.size(); i++) {
      ASSERT_MSG(
          !pc_mems[i]->module.empty(),
  ...
  } else { /* module information is disregarded; here we get the first match
              with app_pc we do not try to see whether there are more than one
              match*/
    for (int i = 0; i < pc_mems.size(); i++) {
      if (pc_mems[i]->pc == app_pc) {
        return pc_mems[i];
...
void update_mem_regions(vector<pc_mem_region_t*>& pc_mems, mem_input_t* input)
{
  ...
  pc_mem_region_t* mem_region
      = get_pc_mem_region(pc_mems, input->module, input->pc);
...
vector<mem_info_t*>
merge_mem_regions(vector<mem_info_t*> first_mem, vector<mem_info_t*> second_mem)
{
  ...
  for (int i = 0; i < first_mem.size(); i++) {
    for (int j = 0; j < second_mem.size(); j++) {
      ...
      if (first->type == second->type) {
        ...
        /* if there is an overlap then second is deleted and merged into the
           first */
...
void print_mem_layout(ostream& file, vector<pc_mem_region_t*>& pc_mems)
{
  ...
  for (int i = 0; i < pc_mems.size(); i++) {
    ...
    file << "app_pc - " << dec << pc_mems[i]->pc << " mem regions - "
         << pc_mems[i]->regions.size() << endl;
    print_mem_layout(
        file,
        pc_mems[i]->regions); /* print mem layout has the seperation line */
  }
}

bool random_dest_select(
    vector<pc_mem_region_t*>& pc_mems, string module, uint32_t app_pc,
    uint64_t* dest, uint32_t* stride)
{
...
bool random_dest_select(
    vector<mem_info_t*>& mem_info, uint64_t* dest, uint32_t* stride)
{
  ...
  for (int i = 0; i < mem_info.size(); i++) {
    if ((mem_info[i]->type == MEM_HEAP_TYPE)
        && ((mem_info[i]->direction & MEM_OUTPUT) == MEM_OUTPUT)) {
      size_heap++;
    }
  }
  ...
  /* select a random location to track using the heap regions (actually non
   * stack regions) */
  for (int i = 0; i < mem_info.size(); i++) {
    if ((mem_info[i]->type == MEM_HEAP_TYPE)
        && ((mem_info[i]->direction & MEM_OUTPUT) == MEM_OUTPUT)) {
      ...
      current_heap++;
      if (current_heap < size_heap / 2)
        continue;

      uint32_t probable_stride
          = get_most_probable_stride(mem_info[i]->stride_freqs);

      uint32_t size
          = (mem_info[i]->end - mem_info[i]->start) / (probable_stride);

      uint32_t random = size / 2 + 10;

      /* here we are returning the first heap block we get; we can also
       * randomize this */
      *dest = mem_info[i]->start + random * probable_stride;
      *stride = probable_stride;

      DEBUG_PRINT(
          ("size, random, start - %d, %d, %lu\n", size, random,
           mem_info[i]->start),
          1);
...
void print_mem_layout(ostream& file, vector<mem_info_t*>& mem_info)
{
  ...
  for (int i = 0; i < mem_info.size(); i++) {
    ...
    /* print out the type */
    if (info->type == MEM_HEAP_TYPE)
      file << "type - heap" << endl;
    if (info->type == MEM_STACK_TYPE)
      file << "type - stack" << endl;

    /* print out the direction */
    if ((info->direction & MEM_INPUT) == MEM_INPUT)
      file << "dir - input" << endl;
    if ((info->direction & MEM_OUTPUT) == MEM_OUTPUT)
      file << "dir - output\n" << endl;
    ...
    for (int j = 0; j < info->stride_freqs.size(); j++) {
      file << "stride - " << dec << info->stride_freqs[j].first << ", freq - "
           << info->stride_freqs[j].second << endl;
    }

    file << "estimated size - " << dec << (info->end - info->start) / (stride)
         << endl;
...
/*
Here, we check whether a particular pc accesses two coalesced memory regions ->
if yes we can merge these two memory regions.
*/
vector<vector<mem_info_t*>> get_merge_opportunities(
    vector<mem_info_t*> mem_info, vector<pc_mem_region_t*> pc_mems)
{
  ...
  for (int i = 0; i < pc_mems.size(); i++) {
    ...
    for (int j = 0; j < mem_info.size(); j++) {
      for (int k = 0; k < pc_mem_info.size(); k++) {
        if (is_overlapped(
                pc_mem_info[k]->start, pc_mem_info[k]->end, mem_info[j]->start,
                mem_info[j]->end)) {
    ...
    for (int j = 1; j < overlapped.size(); j++) {
      ...
      uint32_t first_extent
          = get_extents(overlapped[j - 1], first_dim, first_dim);
      uint32_t first_stride
          = get_stride(overlapped[j - 1], first_dim, first_dim);
      uint32_t right_first
          = overlapped[j - 1]->end + first_extent * first_stride;

      uint32_t second_dim = get_number_dimensions(overlapped[j]);
      uint32_t second_extent
          = get_extents(overlapped[j], second_dim, second_dim);
      uint32_t second_stride
          = get_stride(overlapped[j], second_dim, second_dim);
      uint32_t left_second
          = overlapped[j]->start - second_extent * second_stride;
    ...
    for (int k = 0; k < regions.size(); k++) {
      ...
      if (regions[k].size() >= 2) {
        ...
        for (int j = 0; j < ret.size(); j++) {
          ...
          /* 1000 is just a heuristic value of the set size() -> make it dynamic
           * for more resilience*/
          vector<mem_info_t*> temp(1000);
          vector<mem_info_t*>::iterator it;

          it = set_intersection(
              regions[k].begin(), regions[k].end(), ret[j].begin(),
              ret[j].end(), temp.begin());
          if (it != temp.begin()) {
            it = set_union(
                regions[k].begin(), regions[k].end(), ret[j].begin(),
                ret[j].end(), temp.begin());
  ...
  for (int i = 0; i < ret.size(); i++) {
    for (int j = 0; j < ret[i].size(); j++) {
      DEBUG_PRINT(("%lu,", ret[i][j]->start), 2);
...
/* this is for merging regions which may have not coalesced in buffer structure
 * reconstruction; but found through pc analysis */
void merge_mem_regions_pc(
    vector<vector<mem_info_t*>> mergable, vector<mem_info_t*> total_regions)
{

  DEBUG_PRINT(("merging meminfo regions....\n"), 2);
  LOG(log_file, "merging information" << endl);

  /* take the biggest region and expand - assume that the ghost zones would be
   * minimal compared to the actual buffer sizes accessed */
  for (int i = 0; i < mergable.size(); i++) {
    ...
    /* check neighbours and if faraway do not merge; for now if not input do not
     * merge */
    if (merged->direction == MEM_OUTPUT)
      continue;

    LOG(log_file,
        "merge_region before: " << merged->start << " " << merged->end << endl);
    for (int j = 1; j <= dims; j++) {
      uint32_t stride = get_stride(merged, j, dims);
      uint32_t extents = get_extents(merged, j, dims);
      LOG(log_file, "dim " << j << " extent " << extents << " strides "
                           << stride << endl);
    }
    ...
    if (index != 0) {
      ...
      if (dims == 1) {
        ...
        while (merged->start > region_set[0]->start) {
          ...
          new_region->end
              = new_region->start + get_extents(merged, dims - 1, dims);
    ...
    LOG(log_file,
        "merge_region after: " << merged->start << " " << merged->end << endl);
    for (int j = 1; j <= dims; j++) {
      LOG(log_file, "dim " << j << " extent " << get_extents(merged, j, dims)
                           << " strides " << get_stride(merged, j, dims)
                           << endl);
    }
  }
}

/* app_pc is needed when you are merging the regions of a particular
 * pc_mem_region value*/
bool link_mem_regions_greedy_dim(vector<mem_info_t*>& mem, uint32_t app_pc)
{
  ...
  while (ret) {
    ...
    for (int i = 0; i < mem.size(); i++) {
      ...
      if (i + 2 >= mem.size())
        continue; // at least three regions should be connected
      ...
      if (gap_first == gap_second
          && size_match) { /* ok we can now merge the regions */
        ...
        DEBUG_PRINT(
            ("app_pc %x merged indexes from %d to %d\n", app_pc, i, index), 5);
        ...
        LOG(log_file,
            "merged amount : " << new_mem_info->mem_infos.size() << endl);
        ret = true;
      }
    }
  }
...
/* this is to coalesce regions into big input output chuncks; this is inference
 * and may be wrong this is kind of unsound defragmentation
 */
bool link_mem_regions(vector<mem_info_t*>& mem, uint32_t app_pc)
{
  ...
  /*can use a dynamic programming algo with optimizing for the most number of
   * connected regions*/
  ...
  if (mem.size() > 1) {
    ...
    if (max_val > 2) {
      ...
      /*cout << "merging " << dec << max_val << " app_pc " << hex << app_pc <<
      endl; cout << "gap " << dec << dp[max_i][max_j].gap << endl; */

      /*for (int i = 0; i < mem.size(); i++){
              for (int j = 0; j < mem.size(); j++){
                      cout << dp[i][j].amount << "," << dp[i][j].gap << "," <<
      dp[i][j].connection << " ";
              }
              cout << endl;
      }*/
...
static void populate_overlapped_regions(
    pc_mem_region_t* pc_mem, vector<pc_mem_region_t*>& pc_mems)
{
...
pc_mem_region_t*
get_pc_mem_region(vector<pc_mem_region_t*> regions, uint32_t pc)
{
