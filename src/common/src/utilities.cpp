...
std::vector<cmd_args_t*> get_command_line_args(int argc, char** argv)
{
  ...
  for (int i = 1; i < argc; i += 2) {
    ...
    assert(name[0] == '-'); /* this ensures that first command line arg is
                               actually a name*/
...
bool is_overlapped(
    uint64_t start_1, uint64_t end_1, uint64_t start_2, uint64_t end_2)
{
  ...
  bool partial_overlap = ((start_1 >= start_2) && (start_1 <= end_2))
...
vector<double> solve_linear_eq(vector<vector<double>> A, vector<double> b)
{
  ...
  if (rank < N) {
    ...
    print_system(A, b);
    ASSERT_MSG(
        (false), (" equations are not consistent; system may be non linear\n"));
  }
...
{
  double A_arr[4][2] = {{1, 1}, {2, 1}, {3, 1}, {4, 1}};
