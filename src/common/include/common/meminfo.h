...
struct mem_info_t {
  ...
  bool padding_merge; /* heuristic merging of padded regions with
...
void update_mem_regions(
    std::vector<pc_mem_region_t*>& pc_mems, mem_input_t* input); /* done */
void postprocess_mem_regions(std::vector<pc_mem_region_t*>& mem); /* done */
void print_mem_layout(
    std::ostream& file, std::vector<pc_mem_region_t*>& pc_mems); /* done */
bool random_dest_select(
    std::vector<pc_mem_region_t*>& pc_mems, std::string module, uint64_t app_pc,
    uint64_t* dest, uint32_t* stride); /* done */
void link_mem_regions(
    std::vector<pc_mem_region_t*>& pc_mems, uint32_t mode); /* done */
std::vector<mem_info_t*>
extract_mem_regions(std::vector<pc_mem_region_t*>& pc_mems); /* done */
...
pc_mem_region_t*
get_pc_mem_region(std::vector<pc_mem_region_t*> regions, uint32_t pc);

/* mem info related functions */
void update_mem_regions(
    std::vector<mem_info_t*>& mem_info, mem_input_t* input); /* done */
void postprocess_mem_regions(std::vector<mem_info_t*>& mem); /* done */
void print_mem_layout(
    std::ostream& file, std::vector<mem_info_t*>& mem); /* done */
bool random_dest_select(
    std::vector<mem_info_t*>& mem, uint64_t* dest, uint32_t* stride); /* done */
...
void link_mem_regions_dim(
    std::vector<pc_mem_region_t*>& pc_mems, uint32_t mode);
bool link_mem_regions_greedy_dim(
    std::vector<mem_info_t*>& mem, uint32_t app_pc);
void sort_mem_info(std::vector<mem_info_t*>& mem_info);

std::vector<std::vector<mem_info_t*>> get_merge_opportunities(
    std::vector<mem_info_t*> mem_info, std::vector<pc_mem_region_t*> pc_mems);
void merge_mem_regions_pc(
    std::vector<std::vector<mem_info_t*>> mergable,
    std::vector<mem_info_t*> total_regions);
