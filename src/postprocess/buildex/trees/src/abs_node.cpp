...
Abs_Node::Abs_Node(
    Conc_Node* head, Conc_Node* conc_node, vector<mem_regions_t*>& mem_regions)
    : Node(*conc_node)
{

  mem_regions_t* mem = NULL;
  this->mem_info.associated_mem = NULL;
  if (conc_node->symbol->type == MEM_STACK_TYPE
  ...
  if (conc_node == head)
    filter = true;
  else if (conc_node->srcs.size() == 0)
    filter = true;
  else {
    for (int i = 0; i < conc_node->srcs.size(); i++) {
      if (conc_node->srcs[i]->operation == op_indirect)
        filter = true;
    }
  }
  ...
  if (mem != NULL && filter) {
    ...
    // ASSERT_MSG((conc_node == head || conc_node->srcs.size() <= 1), ("ERROR:
    // mem buffers in the head or in the leaves with at most an indirection\n"));
    ...
    case MEM_INPUT:
      this->type = INPUT_NODE;
      break;
    case MEM_OUTPUT:
      this->type = OUTPUT_NODE;
      break;
    case MEM_INTERMEDIATE:
      this->type = INTERMEDIATE_NODE;
      break;
    }
    ...
    mem_regions_t* head_region
        = get_mem_region(head->symbol->value, mem_regions);
    ...
    for (int i = 0; i < mem->dimensions; i++) {
      ...
      || (conc_node->symbol->type == REG_TYPE)) { /*parameters can be here*/
    this->symbol = conc_node->symbol;
...
string Abs_Node::get_mem_string(vector<string> vars)
{
  ...
  string in_string
      = ((this->mem_info.associated_mem->trees_direction & MEM_OUTPUT)
         == MEM_OUTPUT)
            ? "_buf_in"
            : "";
  ...
  for (int i = 0; i < this->mem_info.dimensions; i++) {
    ...
    for (int j = 0; j < this->mem_info.head_dimensions; j++) {
      ...
      if (this->mem_info.indexes[i][j] == 1) {
        ...
        ret += "(" + to_string(this->mem_info.indexes[i][j]) + ")" + " * "
               + vars[j];
        first = false;
      }
    }
    ...
    if (!first) {
      if (this->mem_info.indexes[i][this->mem_info.head_dimensions] != 0) {
        ret += "+"
               + to_string(
                   this->mem_info.indexes[i][this->mem_info.head_dimensions]);
      }
    } else {
      ret += to_string(
          this->mem_info.indexes[i][this->mem_info.head_dimensions]);
...
{
  if ((this->type == INPUT_NODE) || (this->type == OUTPUT_NODE)
      || (this->type == INTERMEDIATE_NODE)) {
    ...
    if (type == OPERATION_ONLY) {
      ...
      if (type == IMMEDIATE_INT) {
        ...
        return abs(symbol->float_value - abs_node->symbol->float_value) < 1e-6;
          ...
          type == INPUT_NODE || type == OUTPUT_NODE
          || type == INTERMEDIATE_NODE) {
        if (node->srcs.size() > 0 && node->srcs[0]->operation == op_indirect)
          return true;
...
string Abs_Node::get_dot_string() { return get_node_string(); }
...
string Abs_Node::get_simpl_string() { throw "not implemented!"; }
