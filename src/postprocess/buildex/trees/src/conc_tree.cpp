...
Conc_Tree::~Conc_Tree() { delete[] frontier; }
...
Node* Conc_Tree::search_node(operand_t* opnd)
{
  ...
  for (int i = 0; i < frontier[hash].amount; i++) {
    ...
    if ((frontier[hash].bucket[i]->symbol->value == value)
        && (frontier[hash].bucket[i]->symbol->width == width)) {
...
void Conc_Tree::add_to_frontier(int hash, Node* node)
{
  ASSERT_MSG(
      ((node->symbol->type != IMM_INT_TYPE)
       && (node->symbol->type != IMM_FLOAT_TYPE)),
      ("ERROR: immediate types cannot be in the frontier\n"));

  ASSERT_MSG(
      (frontier[hash].amount < SIZE_PER_FRONTIER),
      ("ERROR: bucket size is full\n"));
...
{
  ASSERT_MSG(
      ((opnd->type != IMM_INT_TYPE) && (opnd->type != IMM_FLOAT_TYPE)),
      ("ERROR: immediate types cannot be in the frontier\n"));
  ...
  for (int i = 0; i < amount; i++) {
    ...
    if ((frontier[hash].bucket[i]->symbol->value == value)
        && (frontier[hash].bucket[i]->symbol->width
            == width)) { // assumes that there cannot be two Nodes with same
                         // values
  ...
  if (move) {
    ...
    ASSERT_MSG(
        (frontier[hash].amount > 0),
        ("ERROR: at least one element should have been deleted\n"));
...
void Conc_Tree::get_full_overlap_nodes(
    std::vector<Node*>& nodes, operand_t* opnd)
{
  ASSERT_MSG(
      ((opnd->type != IMM_INT_TYPE) && (opnd->type != IMM_FLOAT_TYPE)),
      ("ERROR: immediate types cannot be in the frontier\n"));
  ...
  if (opnd->type == REG_TYPE) {
    ...
    for (int i = 0; i < frontier[hash].amount; i++) {
      ...
      if (((start > opnd->value)
           && (start + width <= opnd->value + opnd->width))
          || ((start >= opnd->value)
              && (start + width < opnd->value + opnd->width))) {
    ...
    for (int i = 0; i < mem_in_frontier.size(); i++) {
      ...
      for (int j = 0; j < frontier[index].amount; j++) {
        ...
        if ((type == MEM_STACK_TYPE) || (type == MEM_HEAP_TYPE)) {
          ...
          /*check whether this memory is fully contained within the current
           * memory operand*/
          if (((start > opnd->value)
               && (start + width <= opnd->value + opnd->width))
              || ((start >= opnd->value)
...
void Conc_Tree::split_partial_overlaps(
    std::vector<std::pair<Node*, std::vector<Node*>>>& nodes, operand_t* opnd,
    uint32_t hash)
{
  for (int i = 0; i < frontier[hash].amount; i++) {
    ...
    if (opnd->type == split_node->symbol->type) {
      ...
      if (((start >= opnd->value)
           && (start <= opnd->value - opnd->width - 1)) /* start within */
          && (start + width > opnd->value + opnd->width)) /*end strictly after*/
      {
        ...
        *second
            = {split_node->symbol->type,
               width - first->width,
               {opnd->value + opnd->width}}; /* changed width to opnd->width */
        ...
        DEBUG_PRINT(
          ...
          (start < opnd->value) /*start strictly before*/
          && ((start + width - 1 >= opnd->value)
              && (start + width - 1
                  <= opnd->value + opnd->width - 1))) /* end within */
      {
        operand_t* first = new operand_t;
        operand_t* second = new operand_t;
        ...
        *second
        ...
        DEBUG_PRINT(
          ...
          && (start + width
              > opnd->value
                    + opnd->width)) /* strictly within start and end */ {
        ...
        *second
            = {split_node->symbol->type,
               ...
               {opnd->value + opnd->width}}; /* width changed to opnd->width */
...
void Conc_Tree::get_partial_overlap_nodes(
    std::vector<std::pair<Node*, std::vector<Node*>>>& nodes, operand_t* opnd)
{
...
/*helper function for adding dependancies  - could have used the node
 * transformations?*/
...
void Conc_Tree::add_address_dependancy(Node* node, operand_t* opnds)
{
  ...
  Conc_Node* indirect_node = new Conc_Node(
      REG_TYPE, 0, 0, 0.0); // reg_type is used here; it doesn't really matter
                            // as this is an operation only node
  ...
  /* [edx + 2] like addresses */
    ...
    Conc_Node* add_node = new Conc_Node(
        REG_TYPE, 0, 4, 0.0); // reg_type is used here; it doesn't really matter
                              // as this is an operation only node
...
void create_call_dependancy(Conc_Tree* tree, Node* node, Func_Info_t* info)
{
  ...
  for (int i = 0; i < node->prev.size(); i++) {
    ...
    if (prev_node->remove_forward_ref_single(node))
      i--;
...
bool Conc_Tree::tree_add_to_frontier(rinstr_t* instr, Node* src)
{
  ...
  if (instr->operation == op_or) {
    for (int k = 0; k < instr->num_srcs; k++) {
      if ((int32_t)instr->srcs[k].value == -1
          && instr->srcs[k].type == IMM_INT_TYPE) {
        add = false;
      }
    }
  }
...
bool Conc_Tree::update_depandancy_backward(
    rinstr_t* instr, cinstr_t* cinstr, Static_Info* info, uint32_t line,
...
#define ASSIGN_OPT
  ...
  if (func_inside) {
    if (info->pc >= func_info[func_index]->start
        && info->pc <= func_info[func_index]->end) {
      return false;
    } else {
      func_inside = false;
    }
  ...
  // TODO: have precomputed nodes for immediate integers -> can we do it for
  // floats as well? -> just need to point to them in future (space
  // optimization)
  Node* head = get_head();
...
#ifdef INDIRECTION
    if ((info->type & Static_Info::INPUT_DEPENDENT_INDIRECT)
        == Static_Info::INPUT_DEPENDENT_INDIRECT) {
      if (instr->dst.addr != NULL) {
        for (int buf = 0; buf < regions.size(); buf++) {
          if (is_overlapped(
                  regions[buf]->start, regions[buf]->end, instr->dst.value,
                  instr->dst.value + instr->dst.width)) {
  ...
  /*first get the partial overlap nodes - if dest is part of the frontier of a
   * wide region it will be part of the nodes returned*/
  ...
  /*get the destination -> the partial overlap may have created the destination
   * if it was contained with in a wide mem region*/
  int hash_dst = generate_hash(&instr->dst);
  DEBUG_PRINT(
      ("dst_hash : %d, frontier amount : %d\n", hash_dst,
       frontier[hash_dst].amount),
      4);
  Node* dst = search_node(&instr->dst);

  /* now get the full overlap nodes */
  /* nodes that contain with in current node - we can delete these by replacing
   * with the current destination node */
  ...
  if (full_overlap_nodes.size() > 0) {
    ...
    for (int i = 0; i < full_overlap_nodes.size(); i++) {
      DEBUG_PRINT(
          ("full overlap - %s\n",
           opnd_to_string(full_overlap_nodes[i]->symbol).c_str()),
          4);
  ...
  if (dst == NULL) {
    ...
    DEBUG_PRINT(
        ("dst - %s : affecting the frontier\n",
         opnd_to_string(dst->symbol).c_str()),
        4);
    if (debug_level >= 6) {
  ...
  for (int i = 0; i < instr->num_srcs; i++) {
    ...
    // first check whether there are existing nodes in the frontier for these
    // sources if there is we know that the same definition of src is used for
    // this, so we can just point to it rather than creating a new node -> space
    // and time efficient
    int hash_src = generate_hash(&instr->srcs[i]);

    DEBUG_PRINT(
        ("src_hash : %d, frontier amount : %d\n", hash_src,
         frontier[hash_src].amount),
        4);
    ...
    // when do we need another node? if node is not present or if the
    // destination matches src ( e.g : i <- i + 1 ) we do not need to check for
    // immediates here as src will point to a brand new Node in that case and
    // hence will not enter the if statement
    if ((src == NULL)
        || (src == dst)) { // I think now we can remove the src == dst check ->
                           // keeping for sanity purposes (why? because we
                           // remove the dst from the frontier)
      src = new Conc_Node(&instr->srcs[i], regions);
      add_node = true;
    ...
    if ((instr->num_srcs == 1)
        && (instr->operation
            == op_assign)) { // this is just an assign then remove the current
                             // node and place the new src node -> compiler
      ...
      for (int j = 0; j < num_references; j++) {
        ...
        // cout << dst << endl;
        // cout << dst->prev[i] << " " << dst->pos[i] << " " <<
        // dst->prev[i]->srcs[dst->pos[i]] << endl;
        ...
        assign_opt = true; // this is here to differentitate between the head
      ...
      if (assign_opt)
        delete dst; // we have broken all linkages, so just delete it
    }
#endif
    ...
    /* update the frontiers - include the sources to the frontier if new nodes
     * created */
    if (add_node) {
      ...
      if (src_conc->region != NULL) {
        if (head_conc->region != NULL
            && head_conc->region == src_conc->region) {
          recursive = true;
        }
      } else {
    ...
    if ((info->type & Static_Info::INPUT_DEPENDENT_INDIRECT)
        == Static_Info::INPUT_DEPENDENT_INDIRECT) {
      if (instr->srcs[i].addr
          != NULL) { // ok, we have an address calculation dependancy
        // make sure this is a buffer
        for (int buf = 0; buf < regions.size(); buf++) {
          if (is_overlapped(
                  regions[buf]->start, regions[buf]->end, instr->srcs[i].value,
                  instr->srcs[i].value + instr->srcs[i].width)) {
...
bool Conc_Tree::update_depandancy_forward_with_src(
    rinstr_t* instr, uint32_t pc, std::string disasm, uint32_t line,
    bool* src_dep)
{
  ...
  for (int i = 0; i < instr->num_srcs; i++) {
    ...
    for (int j = 0; j < srcs.size(); j++) {
      ...
      if (srcs[j]->type == IMM_FLOAT_TYPE || srcs[j]->type == IMM_INT_TYPE)
        continue;
      if (srcs[j]->type == REG_TYPE && srcs[j]->value == 0)
        continue; // NULL registers
      ...
      if ((src_node != NULL) || (full_overlaps.size() > 0)
          || (partial_overlaps.size() > 0)) {
        if (!ret) {
          Node* dst_node = search_node(&instr->dst);
          if (dst_node == NULL) {
            add_to_frontier(
                generate_hash(&instr->dst), new Conc_Node(&instr->dst));
          }
          ret = true;
        }
        src_dep[i] = true;
...
bool Conc_Tree::update_dependancy_forward(
    rinstr_t* instr, uint32_t pc, std::string disasm, uint32_t line)
{
  for (int i = 0; i < instr->num_srcs; i++) {
    ...
    if (instr->srcs[i].type == IMM_FLOAT_TYPE
        || instr->srcs[i].type == IMM_INT_TYPE)
      continue;
    ...
    if ((src_node != NULL) || (full_overlaps.size() > 0)
      ...
      /*Node * dst_node = search_node(&instr->dst);
      if (dst_node == NULL){
              add_to_frontier(generate_hash(&instr->dst), new
      Conc_Node(&instr->dst));
      }*/
...
bool Conc_Tree::update_dependancy_forward_with_indirection(
    rinstr_t* instr, uint32_t pc, std::string disasm, uint32_t line)
{
...
void Conc_Tree::print_conditionals()
{
  ...
  for (int i = 0; i < conditionals.size(); i++) {
    cout << conditionals[i]->jump_info->cond_pc << " "
         << conditionals[i]->line_cond << endl;
  }
}

void Conc_Tree::number_parameters(
    Node* node, vector<mem_regions_t*> mem_regions)
{
  ...
  if (node->srcs.size() == 0) {
    if (conc_node->symbol->type == REG_TYPE) {
      if (conc_node->para_num == -1) {
        ...
        conc_node->symbol->type == MEM_HEAP_TYPE
        || conc_node->symbol->type == MEM_STACK_TYPE) {
...
std::string Conc_Tree::serialize_tree() { throw "not implemented!"; }

void Conc_Tree::construct_tree(std::string stree) { throw "not implemented!"; }
