...
Conc_Node::Conc_Node(operand_t* symbol, vector<mem_regions_t*>& regions)
    : Conc_Node(symbol)
{
...
Conc_Node::Conc_Node(
    uint32_t type, uint64_t value, uint32_t width, float float_value)
{
...
Conc_Node::~Conc_Node() { delete this->symbol; }
...
bool Conc_Node::are_nodes_similar(Node* node)
{
  ...
  if (node->srcs.size() > 0) {
    return (operation == node->operation) && (srcs.size() == node->srcs.size());
  } else {
...
string Conc_Node::get_dot_string() { return get_node_string(); }

string Conc_Node::get_simpl_string() { throw "not implemented!"; }
