...
Node* abs_node_from_conc_node(void* head, void* node, void* peripheral_data)
{
  ...
  Abs_Node* new_node = new Abs_Node(
      static_cast<Conc_Node*>(head), static_cast<Conc_Node*>(node), regions);
  return new_node;
}

void Abs_Tree::build_abs_tree_unrolled(
...
void Abs_Tree::build_abs_tree_exact(
    Conc_Tree* tree, std::vector<mem_regions_t*>& mem_regions)
{
...
void Abs_Tree::seperate_intermediate_buffers() { throw "not implemented!"; }

std::string Abs_Tree::serialize_tree() { throw "not implemented!"; }

void Abs_Tree::construct_tree(std::string stree) { throw "not implemented!"; }

bool check_node_repitition(vector<Abs_Node*>& stack, Abs_Node* node)
{

  ASSERT_MSG(
      ((node->type == Abs_Node::OUTPUT_NODE)
       || (node->type == Abs_Node::INTERMEDIATE_NODE)),
      ("ERROR: the node should be an intermediate or an output buffer\n"));

  for (int i = 0; i < stack.size(); i++) {
    ASSERT_MSG(
        (stack[i]->type != Abs_Node::INPUT_NODE),
        ("ERROR: input nodes should be immediately removed from the stack at "
         "the exit\n"));
    if ((node->type == stack[i]->type)
        && (node->mem_info.dimensions == stack[i]->mem_info.dimensions)) {
      bool same
          = (stack[i]->mem_info.associated_mem
...
bool check_recursive_tree(vector<Abs_Node*>& stack, Abs_Node* node)
{
  ...
  if (node->type == Abs_Node::OUTPUT_NODE
      || node->type == Abs_Node::INTERMEDIATE_NODE) {
  ...
  traverse_tree(
      ...
      [](Node* node, void* value) -> void* {
        ...
        if (abs_node->type == Abs_Node::PARAMETER) {
          vector<Abs_Node*>* nodes = (vector<Abs_Node*>*)value;
          for (int i = 0; i < (*nodes).size(); i++) {
            if ((*nodes)[i]->para_num == abs_node->para_num) {
              return NULL;
            }
          }
          (*nodes).push_back(abs_node);
...
vector<Abs_Node*> Abs_Tree::get_buffer_region_nodes()
{
  ...
  traverse_tree(
      ...
      [](Node* node, void* value) -> void* {
        ...
        if (abs_node->type == Abs_Node::INPUT_NODE
            || abs_node->type == Abs_Node::OUTPUT_NODE
            || abs_node->type == Abs_Node::INTERMEDIATE_NODE) {
          vector<Abs_Node*>* nodes = (vector<Abs_Node*>*)value;
          for (int i = 1; i < (*nodes).size();
               i++) { /* trick to get rid of the head node */
            if (abs_node->mem_info.associated_mem
                == (*nodes)[i]->mem_info.associated_mem) {
              return NULL;
            }
          }
          (*nodes).push_back(abs_node);
  ...
  traverse_tree(
      ...
      [](Node* node, void* value) -> void* {
        ...
        if (abs_node->type == Abs_Node::INPUT_NODE
            || abs_node->type == Abs_Node::OUTPUT_NODE
            || abs_node->type == Abs_Node::INTERMEDIATE_NODE) {
          if (abs_node->mem_info.dimensions > *max_dim)
            *max_dim = abs_node->mem_info.dimensions;
        }
...
void Abs_Tree::print_dot_algebraic(
    ...
    std::vector<std::string> vars)
{
  ...
  ASSERT_MSG(
      (vars.size() > 0),
      ("ERROR: please use the non-var version for printing\n"));
      ...
      [](Node* node, void* value) -> void* {
        if (node->visited == false) {
          ...
          vector<string> vec_vars(
              (*vec_string).begin() + 1, (*vec_string).end());

          (*vec_string)[0]
              += dot_get_node_string(
                     abs_node->order_num, abs_node->get_dot_string(vec_vars))
                 + "\n";
          node->visited = true;
        }
