...
void Tree::set_head(Node* node) { head = node; }
...
Node* Tree::get_head() { return head; }

/* pre-order traversal of the tree */
void* Tree::traverse_tree(
    Node* node, void* value, node_mutator mutator, return_mutator ret_mutator)
{
  ...
  for (int i = 0; i < node->srcs.size(); i++) {
    traverse_val.push_back(
        traverse_tree(node->srcs[i], value, mutator, ret_mutator));
  }
...
void Tree::simplify_tree() /* simplification routines are not written at the
                              moment */
{
}
  ...
  traverse_tree(
      ...
      [](Node* node, void* value) -> void* {
        if (node->visited == false) {
          ...
          *node_string
              += dot_get_node_string(node->order_num, node->get_dot_string())
                 + "\n";
          node->visited = true;
        }
  ...
  traverse_tree(
      ...
      [](Node* node, void* value) -> void* {
        if (node->visited == false) {
          ...
          for (int i = 0; i < node->srcs.size(); i++) {
            *edge_string += dot_get_edge_string(
                                node->order_num, node->srcs[i]->order_num)
                            + "\n";
          }
          node->visited = true;
        }
...
void Tree::print_tree_recursive(Node* node, std::ostream& file)
{
  ...
  if (no_srcs == 0) { /* we are at a leaf just print it*/
    ...
    /* we can have a full overlap - partial overlaps will have more than one
     * operand */
    if (node->operation == op_full_overlap) {
      file << "{" << opnd_to_string(node->symbol) << " -> "
           << opnd_to_string(node->srcs[0]->symbol) << "}";
      file << "(";
    ...
    // ASSERT_MSG((node->operation == op_partial_overlap), ("ERROR: unexpected
    // operation with more than two srcs\n"));
...
void Tree::print_tree(std::ostream& file) { print_tree_recursive(head, file); }
...
void Tree::copy_exact_tree_structure(
    Tree* tree, void* peripheral_data, node_to_node node_creation)
{
  /* assumption the tree is numbered and the visited state is cleared */
  ASSERT_MSG(
      (tree->get_head()->order_num != -1),
      ("ERROR: the concrete tree is not numbered\n"));
  ASSERT_MSG(
      (tree->get_head()->visited == false),
  ...
  traverse_tree(
      tree->get_head(), &tree_map,
      [](Node* node, void* value) -> void* {
        vector<pair<Node*, vector<int>>>* map
            = (vector<pair<Node*, vector<int>>>*)value;
...
void Tree::copy_unrolled_tree_structure(
    Tree* tree, void* peripheral_data, node_to_node node_creation)
{

  set_head(node_creation(tree->get_head(), tree->get_head(), peripheral_data));
  copy_unrolled_tree_structure(
      tree->get_head(), tree->get_head(), get_head(), peripheral_data,
      node_creation);
}

void Tree::copy_unrolled_tree_structure(
    Node* head, Node* from, Node* to, void* peripheral_data,
    node_to_node node_creation)
{
  ...
  for (int i = 0; i < from->srcs.size(); i++) {
    ...
    copy_unrolled_tree_structure(
        head, from->srcs[i], new_node, peripheral_data, node_creation);
  }
}
...
void Tree::change_head_node()
{
  if (head->operation == op_assign) { /* then this node can be removed */
    ASSERT_MSG(
        (head->srcs.size() == 1), ("ERROR: unexpected number of operands\n"));
...
{
  if (!Node::are_nodes_similar(node))
    return false;
  ...
  traverse_tree(
      ...
      [](Node* dst, void* value) -> void* {
        ...
        if (dst->operation == op_assign && dst != head
          ...
          ASSERT_MSG(
              (dst->srcs.size() == 1),
              ("ERROR: assign should only have one source\n"));
          Node* src = dst->srcs[0];
...
std::vector<mem_regions_t*>
Tree::identify_intermediate_buffers(std::vector<mem_regions_t*> mem)
{
...
bool Tree::is_recursive(Node* node, vector<mem_regions_t*>& regions)
{
  ...
  if (head != node) {
    ...
    if (node->symbol->type == MEM_HEAP_TYPE
        || node->symbol->type == MEM_STACK_TYPE) {
      if (get_mem_region(head->symbol->value, regions)
          == get_mem_region(node->symbol->value, regions)) {
        return true;
      }
    }
  }
...
bool remove_multiplication_internal(Node* dst)
{
  ...
  if (dst->visited)
    ...
    if (dst->operation == op_mul) {
      ...
      for (int i = 0; i < dst->srcs.size(); i++) {
        if (dst->srcs[i]->symbol->type == IMM_INT_TYPE) {
          imm_value = dst->srcs[i]->symbol->value;
          index = i;
          break;
        }
      }
    ...
    for (int i = 0; i < dst->srcs.size(); i++) {
      if (remove_multiplication_internal(dst->srcs[i]))
        i--;
    }
  ...
  /*traverse_tree(head, head, [](Node * dst, void * value)->void*{
          ...
          if (!dst->visited){
                  ...
                  if (dst->operation == op_mul){
                          ...
                          for (int i = 0; i < dst->srcs.size(); i++){
                                  if (dst->srcs[i]->symbol->type ==
  IMM_INT_TYPE){ imm_value = dst->srcs[i]->symbol->value; index = i; break;
                                  }
                          }

                          if (index != -1 && imm_value < 10 && imm_value >= 0){
                                  for (int i = 0; i < dst->prev.size(); i++){
                                          for (int j = 0; j < dst->srcs.size();
  j++){ if (index != j){ for (int k = 0; k < imm_value; k++){
                                                                  dst->prev[i]->add_forward_ref(dst->srcs[j]);
                                                          }
                                                  }
                                          }
      ...
      [](Node* node, void* value) -> void* {
        if (node->operation == op_mul || node->operation == op_add) {
          ...
          if (indexes.size() > 0) {
            ...
            ASSERT_MSG(
                indexes[0] == 0,
                ("ERROR: canonicalized tree has immediates to the left\n"));
...
bool remove_redundant_internal(Node* node)
{
  ...
  for (int i = 0; i < node->srcs.size(); i++) {
    if (remove_redundant_internal(node->srcs[i]))
      i = (i - 1 >= 0) ? i - 1 : 0;
  }
...
void Tree::remove_redundant_nodes() { remove_redundant_internal(head); }
...
bool convert_node_sub_internal(Node* node, Node* head)
{
  ...
  for (int i = 0; i < node->srcs.size(); i++) {
    if (convert_node_sub_internal(node->srcs[i], head))
      i--;
  }
  return changed;
}
  ...
  traverse_tree(
      ...
      [](Node* node, void* value) -> void* {
        if (node->operation == op_add) {
          ...
          for (int i = 0; i < node->srcs.size(); i++) {
            if (node->srcs[i]->minus)
              negative.push_back(node->srcs[i]);
            else
              positive.push_back(node->srcs[i]);
          }

          // cout << "minus " << positive.size() << " " << negative.size() <<
          // endl;
      ...
      [](Node* node, void* value) -> void* {
        if (node->operation == op_or) {
          ...
          for (int i = 0; i < node->srcs.size(); i++) {
            ...
            if (node->srcs[i]->symbol->type == IMM_INT_TYPE
                && (int32_t)node->srcs[i]->symbol->value == -1) {
              index = i;
              break;
            }
          }
...
void Tree::mark_recursive()
{
  ...
  traverse_tree(
      ...
      [](Node* node, void* value) -> void* {
        ...
        if (head_region == conc_region
            && head->symbol->value != node->symbol->value) {
          tree->recursive = true;
        }
