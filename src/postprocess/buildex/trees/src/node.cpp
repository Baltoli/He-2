...
bool Node::remove_forward_ref_single(Node* ref)
{
  ...
  if (erase) {
    for (int i = 0; i < this->srcs.size(); i++) {
      ...
      if (count == 0) {
        node_counts.push_back(make_pair(this->srcs[i], 1));
      }
      int found = 0;
...
void Node::safely_delete(Node* head)
{
  if ((this->prev.size() == 0) && (this != head)) {
    /* remove any backward references to this node if existing ; we are assuming
     * that there can only be one head node for the tree */
    /* no need to remove the forward reference as it is only within the
     * deletable node */
    for (int i = 0; i < this->srcs.size(); i++) {
...
bool sort_function(
    pair<Node*, uint32_t> first_node, pair<Node*, uint32_t> second_node)
{
...
bool Node::congregate_node(Node* head)
{
  ...
  for (int i = 0; i < (int)this->prev.size(); i++) {
    ...
    if ((is_operation_associative(this->operation))
        && (this->operation == this->prev[i]->operation)) {
...
void Node::order_node()
{
  ...
  for (int i = 0; i < imm.size(); i++) {
    ...
    for (int j = 0; j < imm[i].first->prev.size(); j++) {
      if (imm[i].first->prev[j] == this
          && imm[i].first->pos[j] == imm[i].second) {
        imm[i].first->pos[j] = i;
      }
    }
  }
  ...
  for (int i = 0; i < other.size(); i++) {
    ...
    for (int j = 0; j < other[i].first->prev.size(); j++) {
      if (other[i].first->prev[j] == this
          && other[i].first->pos[j] == other[i].second) {
  ...
  for (int i = 0; i < heap.size(); i++) {
    ...
    for (int j = 0; j < heap[i].first->prev.size(); j++) {
      if (heap[i].first->prev[j] == this
          && heap[i].first->pos[j] == heap[i].second) {
