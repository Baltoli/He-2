...
public:
  ...
  std::vector<uint>
      pos; /* position of the parent node's srcs list for this child */
...
public:
  ...
  /* following routines will have concrete implementations; but can be
   * overloaded */
  ...
  void remove_intermediate_ref(
      Node* dst, Node* src); /* (dst -> node -> src)  => (dst -> src) */
...
public:
  ...
  bool congregate_node(
      Node* head); /* congregate nodes with associative operations */
...
abs_nodes types and processing are determined by the Node symbol types and the
mem_regions types, therefore, we can see as building the abs tree as a
unification of the information extracted by the instrace(building the concrete
tree) and memdump(locating image locations and properties). Further abstraction
can be performed on this tree.

type(Abs_node) <- unify ( type(Node->symbol) , type(mem_region) )

Next, we assume that the bottom of the tree should be Heap Nodes (image
dependancies), Stack or Regs(parameters) or immediate ints, immediate floats
...
-> dimensions - the dimensionality of the mem_region -> this depends on the
image layout
-> indexes - this carries the final abstracted out index coefficients when we
express the filter algebrically
-> pos - this lifts the flat memory address into coordinates in the dimensional
space *type will depend on whether the associated mem region is input, output or
intermediate
...
public:
  ...
  Abs_Node();
  Abs_Node(
      Conc_Node* head, Conc_Node* conc_node,
