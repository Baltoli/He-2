...
typedef void* (*return_mutator)(
    void* node_value, std::vector<void*> traverse_value, void* value);
...
private:
  ...
  static void copy_unrolled_tree_structure(
...
protected:
  void* traverse_tree(
      Node* node, void* value, node_mutator mutator,
      return_mutator ret_mutator);
  void copy_exact_tree_structure(
      Tree* tree, void* peripheral_data, node_to_node node_creation);
  void copy_unrolled_tree_structure(
      Tree* tree, void* peripheral_data, node_to_node node_creation);

public:
  uint32_t num_nodes;
  int32_t tree_num; /* this is for numbering the tree (most porabably based on
                       output location - used in tree clustering) */
  ...
  /* for tree simplification -> each type of tree will have its own
  ...
  identify_intermediate_buffers(std::vector<mem_regions_t*> mem);
...
private:
  ...
  frontier_t* frontier; /*this is actually a hash table keeping pointers to the
                           Nodes already allocated */
  std::vector<uint32_t> mem_in_frontier; /* memoization structures for partial
                                            mem and reg writes and reads */
  ...
  void split_partial_overlaps(
      std::vector<std::pair<Node*, std::vector<Node*>>>& nodes, operand_t* opnd,
      uint32_t hash);
  void get_partial_overlap_nodes(
      std::vector<std::pair<Node*, std::vector<Node*>>>& nodes,
      operand_t* opnd);

  void number_parameters(Node* node, vector<mem_regions_t*> regions);

public:
  /* this governs which conditionals affect the tree -> needed for predicate
   * tree generation */
  ...
  // bool update_depandancy_backward(rinstr_t * instr, cinstr_t * cinstr,
  // Static_Info * info, uint32_t line);
  bool update_depandancy_backward(
      rinstr_t* instr, cinstr_t* cinstr, Static_Info* info, uint32_t line,
      vector<mem_regions_t*> region, vector<Func_Info_t*> func_info);
  bool update_dependancy_forward(
      rinstr_t* instr, uint32_t pc, std::string disasm, uint32_t line);
  bool update_dependancy_forward_with_indirection(
      rinstr_t* instr, uint32_t pc, std::string disasm, uint32_t line);
  bool update_depandancy_forward_with_src(
      rinstr_t* instr, uint32_t pc, std::string disasm, uint32_t line,
      bool* src_dep);
...
public:
  ...
  void build_abs_tree_exact(
      Conc_Tree* tree, std::vector<mem_regions_t*>& mem_regions);
  void build_abs_tree_unrolled(
      Conc_Tree* tree, std::vector<mem_regions_t*>& mem_regions);
  ...
  void print_dot_algebraic(
      std::ostream& file, std::string name, uint32_t number,
...
private:
  static void build_compound_tree_unrolled(
      Comp_Abs_Node* comp_node, std::vector<Abs_Node*> abs_nodes);
  void traverse_trees(
      std::vector<Abs_Node*> abs_nodes,
      std::vector<std::pair<std::vector<Abs_Node*>, std::vector<int>>>&
          node_pos);
