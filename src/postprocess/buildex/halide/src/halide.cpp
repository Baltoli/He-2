...
/*halide header and footer - in future move to a file and read the header from
 * the file*/
...
string print_Halide_footer() { return "return 0;\n}"; }

/* halide supported types - Int, UInt , Float with width specified as the first
...
string print_Halide_output_to_file(
    Abs_Node* node, string filename, uint32_t file_no, string vector)
{

  return node->mem_info.associated_mem->name + ".compile_to_file(\"" + filename
         + "_" + to_string(file_no) + "\"," + vector + ");";
}
...
string print_Halide_input_declaration(Abs_Node* node)
{
  ...
  // ASSERT_MSG((node->type == Abs_Node::INPUT_NODE || node->type ==
  // Abs_Node::INTERMEDIATE_NODE), ("ERROR: the node cannot be a output"));

  // cout << node->mem_info.associated_mem->name << " " <<
  // node->mem_info.associated_mem->trees_direction << endl;

  if ((node->mem_info.associated_mem->trees_direction & MEM_INPUT) != MEM_INPUT)
    return "";

  string in_string
      = ((node->mem_info.associated_mem->trees_direction & MEM_OUTPUT)
         == MEM_OUTPUT)
            ? "_buf_in"
            : "";
...
string print_Halide_parameter_declaration(Abs_Node* node)
{
  ...
  ASSERT_MSG(
      (node->type == Abs_Node::PARAMETER),
      ("ERROR: the node is not a parameter\n"));
...
Halide_Program::Func* Halide_Program::check_function(mem_regions_t* mem)
{
  ...
  for (int i = 0; i < funcs.size(); i++) {
    ...
    if (funcs[i]->pure_trees.size() > 0) {
      Abs_Node* node
          = static_cast<Abs_Node*>(funcs[i]->pure_trees[0]->get_head());
...
void Halide_Program::sort_functions()
{
  ...
  for (int i = 0; i < funcs.size(); i++) {
    sort(
        funcs[i]->pure_trees.begin(), funcs[i]->pure_trees.end(), compare_tree);
    for (int j = 0; j < funcs[i]->reduction_trees.size(); j++) {
      sort(
    ...
    if (rdom->type == cur_rdom->type) {
      if (rdom->type == INDIRECT_REF) {
        ...
        ASSERT_MSG(
            ...
            ("ERROR: reduction domain dimensions for the same buffer should be "
             "the same"));
...
void Halide_Program::populate_red_funcs(
    Abs_Tree* tree, std::vector<std::pair<int32_t, int32_t>> boundaries,
    Abs_Node* node)
{
...
void Halide_Program::populate_paras_input_paras(bool para)
{
  ...
  for (int i = 0; i < funcs.size(); i++) {
    trees.insert(
        trees.end(), funcs[i]->pure_trees.begin(), funcs[i]->pure_trees.end());
    for (int j = 0; j < funcs[i]->reduction_trees.size(); j++) {
      trees.insert(
          trees.end(), funcs[i]->reduction_trees[j].second.begin(),
  ...
  for (int i = 0; i < trees.size(); i++) {
    ...
    temp = para ? trees[i]->retrieve_parameters()
                : trees[i]->get_buffer_region_nodes();
  ...
  if (para) {
    ...
    for (int i = 0; i < total.size(); i++) {
      for (int j = 0; j < total.size(); j++) {
        if (i == j)
          continue;
        if (total[i]->mem_info.associated_mem
            == total[j]->mem_info.associated_mem) {
          total.erase(total.begin() + i--);
          break;
        }
      }
    ...
    for (int i = 0; i < total.size(); i++) {
      ...
      for (int j = 0; j < output.size(); j++) {
        if (total[i]->mem_info.associated_mem
            == output[j]->mem_info.associated_mem) {
          if (total[i]->is_node_indirect() != -1)
            total[i]->mem_info.associated_mem->trees_direction
                &= ~((uint32_t)MEM_INPUT);
          break;
        }
      }
...
void Halide_Program::populate_params() { populate_paras_input_paras(true); }
...
string Halide_Program::print_full_overlap_node(
    Abs_Node* node, Node* head, vector<string> vars)
{
  ...
  /* BUG - overlap_end == node_end ? this is not always true if mem and reg
   * values are*/

  string ret = "";

  if (node->symbol->width
      == overlap->symbol
             ->width) { /* where the nodes are of reg and memory etc.*/
  ...
  /*if(node->symbol->type == REG_TYPE && overlap->symbol->type == REG_TYPE &&
  overlap_end != node_end){ ret += " ( ( "; ret += print_abs_tree(overlap,head,
  vars); ret += " >> " + to_string((overlap_end - node_end) * 8) + " ) "; ret +=
  " ) & " + to_string(mask);
  }
  else{*/
  ret += " ( ";
...
string Halide_Program::print_partial_overlap_node(
    Abs_Node* node, Node* head, vector<string> vars)
{
  ...
  /*for (int i = 0; i < node->srcs.size(); i++)
  {
          ...
          uint32_t overlap_end = overlap->symbol->value +
  overlap->symbol->width; uint32_t node_end = node->symbol->value +
  node->symbol->width;
...
void Halide_Program::print_halide_program(
    std::ostream& out, vector<string> red_variables)
{
  ...
  for (int i = 0; i < params.size(); i++) {
    out << print_Halide_argument(arg, "p_" + to_string(params[i]->para_num))
        << ";" << endl;
  }
  for (int i = 0; i < inputs.size(); i++) {
    out << print_Halide_argument(arg, inputs[i]->mem_info.associated_mem->name)
        << ";" << endl;
  }
...
string print_select_statement(
    Halide_Program::Select_Expr* current_expr,
    Halide_Program::Select_Expr* next_expr)
{

  if (next_expr != NULL) { /* we have a false value*/
    return "Expr " + current_expr->name + " = select(" + current_expr->condition
           + "," + current_expr->truth_value + "," + next_expr->name + ");\n";
  } else {
...
string Halide_Program::print_predicated_tree(
    vector<Abs_Tree*> trees, string expr_tag, vector<string> vars)
{
  ...
  for (int i = 0; i < trees.size(); i++) {
    ...
    expr->name = print_expression_name(
        head->mem_info.associated_mem->name + expr_tag, i);
    expr->condition
        = print_conditional_trees(trees[i]->conditional_trees, vars);
    expr->truth_value
        = print_abs_tree(trees[i]->get_head(), trees[i]->get_head(), vars);
  ...
  output += print_output_func_def(
      static_cast<Abs_Node*>(trees[0]->get_head()), vars);

  Abs_Node* head_node = static_cast<Abs_Node*>(trees[0]->get_head());

  uint32_t clamp_max = min(
      (uint32_t(~0)) >> (32 - head_node->symbol->width * 8), (uint32_t)65535);
  uint32_t clamp_min = 0;

  /* BUG - what to do with the sign?? */
  output += " = " + get_cast_string(head_node, false) + "( clamp("
            + exprs[0]->name + "," + to_string(clamp_min) + ","
            + to_string(clamp_max) + ") );\n";
...
std::string
Halide_Program::print_red_trees(Func* func, vector<string> red_variables)
{

  /* Assumption - If the RDom is the same, then the trees are different due to
  conditionals. If the RDom's are not the same, then those trees are computed
  without overlap */

  ASSERT_MSG(
      (func->pure_trees.size() > 0),
      ("ERROR: reduction updates should have initial pure definitions\n"));

  if (func->reduction_trees.size() == 0)
    return "";
  ...
  for (int i = 0; i < func->reduction_trees.size(); i++) {
    ...
    ret += print_predicated_tree(
        func->reduction_trees[i].second, "_r" + to_string(i) + "_",
        get_reduction_index_variables(name));
  }
...
std::string
Halide_Program::print_function(Func* func, vector<string> red_variables)
{
...
/* very crude printing */
std::string Halide_Program::get_indirect_string(
    Abs_Node* node, Abs_Node* head, vector<string> vars)
{
...
std::string
Halide_Program::print_abs_tree(Node* nnode, Node* head, vector<string> vars)
{
  ...
  if (node->type == Abs_Node::OPERATION_ONLY) {
    ...
    if (node != head) {
      if (indirect) {
        ...
        ret += print_abs_tree(
            node->srcs[pos], head,
            vars); /* assumes that these nodes are at the leaves */
      } else {
      ...
      if (node->operation
          != op_assign) { /* the node contains some other operation */
...
std::string Halide_Program::print_conditional_trees(
    std::vector<std::pair<Abs_Tree*, bool>> conditions, vector<string> vars)
{
...
vector<mem_dump_regions_t*>
Halide_Program::get_memory_regions(vector<string> memdump_files)
{
  ...
  for (int i = 0; i < memdump_files.size(); i++) {
    ...
    ASSERT_MSG(
        stat(memdump_files[i].c_str(), &results) == 0,
        ("file stat collection unsuccessful\n"));
    ...
    for (int j = 0; j < dump.size(); j++) {
      if (!dump[j]->write) {
        if (start >= dump[j]->start && end <= dump[j]->end) {
          ...
          dump_region->values = &dump[j]->buffer[start - dump[j]->start];
          dump_region->size
              = get_actual_size(inputs[i]->mem_info.associated_mem);
          dumps.push_back(dump_region);
          break;
        }
      }
          ...
          for (int k = 0; k < dump.size(); k++) {
            if (!dump[k]->write) {
              if (start >= dump[k]->start && end <= dump[k]->end) {
                ...
                dump_region->size
                    = get_actual_size(head->mem_info.associated_mem);
                dumps.push_back(dump_region);
                break;
              }
            }
