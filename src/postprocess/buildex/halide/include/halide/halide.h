...
/* main halide program for generation Halide code - we also need a helper halide
 * module to setup buffer_t for patching */
...
public:
  ...
  /* input, output and intermediate states that can be modelled as a Halide
   * function*/
  /* when ever there are reduction trees, then pure definitions will act as the
   * initial updates of the function */
  enum RDom_type {
    INDIRECT_REF,
    EXTENTS,
  };
  ...
  std::vector<Abs_Node*>
      output; /* this is used to populate the arguments string */
...
public:
  ...
  void print_halide_program(
      std::ostream& file, std::vector<std::string> red_variables);

  std::vector<mem_dump_regions_t*>
  get_memory_regions(std::vector<std::string> memdump_files);
...
private:
  ...
  /* printing functions */
  std::string
  print_function(Func* func, std::vector<std::string> red_variables);
  std::string print_pure_trees(Func* func);
  std::string
  print_red_trees(Func* func, std::vector<std::string> red_variables);
  std::string print_predicated_tree(
      std::vector<Abs_Tree*> trees, std::string expr_tag,
      std::vector<std::string> vars);

  /* print out sub parts of the Halide function */
  std::string print_abs_tree(Node* node, Node* head, std::vector<string> vars);
  std::string print_conditional_trees(
      std::vector<std::pair<Abs_Tree*, bool>> conditions,
      std::vector<string> vars);

  /* printing full and partial overlap nodes */
  std::string
  print_full_overlap_node(Abs_Node* node, Node* head, std::vector<string> vars);
  std::string print_partial_overlap_node(
      Abs_Node* node, Node* head, std::vector<string> vars);
  ...
  /* very crude change*/
  std::string get_indirect_string(
      Abs_Node* node, Abs_Node* head, std::vector<std::string> vars);
};
