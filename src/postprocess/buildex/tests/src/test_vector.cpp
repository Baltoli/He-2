...
int main(int argc, char** argv)
{
  ...
  ASSERT_MSG(
      (!instrace_filename.empty()),
      ("suitable instrace file cannot be located; please specify manually\n"));
  ...
  vec_cinstr instrs_forward
      = filter_instr_trace(start_pc, end_pc, instrs_forward_unfiltered);
  ...
  for (int i = 0; i < instrs_forward.size(); i++) {
    int amount = 0;
    rinstr_t* rinstr = cinstr_to_rinstrs(
        instrs_forward[i].first, amount, instrs_forward[i].second->disassembly,
        i);
    delete rinstr;
  }
