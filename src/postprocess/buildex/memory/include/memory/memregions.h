...
void mark_possible_buffers(
    ...
    std::vector<mem_regions_t*>& mem_regions, std::vector<Static_Info*>& info,
    vec_cinstr& instrs);
void remove_possible_stack_frames(
    std::vector<pc_mem_region_t*>& pc_mem, std::vector<mem_info_t*>& mem,
    std::vector<Static_Info*>& info, vec_cinstr& instrs);

/* extracting random locations, memregions */
std::vector<uint64_t> get_nbd_of_random_points(
    std::vector<mem_regions_t*> image_regions, uint32_t seed, uint32_t* stride);
std::vector<uint64_t> get_nbd_of_random_points_2(
    std::vector<mem_regions_t*> image_regions, uint32_t seed, uint32_t* stride);
...
mem_regions_t* get_mem_region(
    uint64_t value,
    std::vector<mem_regions_t*>& mem_regions); /* extracting mem regions */
uint64_t get_mem_location(
    std::vector<int> base, std::vector<int> offset, mem_regions_t* mem_region,
    bool* success); /*extracting mem locations - given the D-dimensional
                       coordinates this retrieves the memory address */
std::vector<int> get_mem_position(
    mem_regions_t* mem_region,
    uint64_t mem_value); /* given a memory location get the memory position in
                            D-dimensional co-ordinates */
