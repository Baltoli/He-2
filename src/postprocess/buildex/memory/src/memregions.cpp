...
vector<uint64_t> get_nbd_of_random_points(
    vector<mem_regions_t*> image_regions, uint32_t seed, uint32_t* stride)
{
  ...
  *stride = random_mem_region->bytes_per_pixel;
  ...
  for (int i = -boundary; i <= boundary; i++) {
    ...
    for (int j = 0; j < base.size(); j++) {
      if (j == affected) {
        if (base[j] + i < 0 || base[j] + i >= random_mem_region->extents[j])
          offset.push_back(0);
        else
          offset.push_back(i);
        // offset.push_back(i);
...
vector<uint64_t> get_nbd_of_random_points_2(
    vector<mem_regions_t*> image_regions, uint32_t seed, uint32_t* stride)
{
  ...
  *stride = random_mem_region->bytes_per_pixel;
  ...
  for (int i = -boundary; i <= boundary; i++) {
    ...
    for (int j = 0; j < base.size(); j++) {
      if (j == affected) {
        val[j]++;
        if (val[j] + base[j] < random_mem_region->extents[j]
            && val[j] + base[j] > 0) {
...
mem_regions_t* get_random_output_region(vector<mem_regions_t*> regions)
{
  ...
  for (int i = 0; i < regions.size(); i++) {
    if (regions[i]->direction == MEM_INTERMEDIATE
        || regions[i]->direction == MEM_OUTPUT) {
      no_regions++;
    }
  }
...
uint64_t get_mem_location(
    vector<int> base, vector<int> offset, mem_regions_t* mem_region,
    bool* success)
{

  ASSERT_MSG(
      (base.size() == mem_region->dimensions),
      ("ERROR: dimensions dont match up\n"));
...
vector<int> get_mem_position(mem_regions_t* mem_region, uint64_t mem_value)
{
  ...
  // cout << "mem position: " << dec << offset << " start " << mem_region->start
  // << " end " << mem_region->end << " value " << mem_value << endl;

  for (int i = mem_region->dimensions - 1; i >= 0; i--) {
    int point_offset = offset / mem_region->strides[i];
    if (point_offset >= mem_region->extents[i]) {
      point_offset = -1;
    }
...
mem_regions_t*
get_mem_region(uint64_t value, vector<mem_regions_t*>& mem_regions)
{
...
void print_mem_regions(ostream& file, mem_regions_t* region)
{
  ...
  case MEM_INPUT:
    file << "image input" << endl;
    break;
  case MEM_OUTPUT:
    file << "image output" << endl;
    break;
  case MEM_INTERMEDIATE:
    file << "image intermediate" << endl;
    break;
  }
...
void remove_possible_stack_frames(
    vector<pc_mem_region_t*>& pc_mem, vector<mem_info_t*>& mem,
    vector<Static_Info*>& info, vec_cinstr& instrs)
{
  ...
  for (int i = 0; i < mem.size(); i++) {
    ...
    for (int j = 0; j < pc_mem.size(); j++) {
      ...
      for (int k = 0; k < mem_info.size(); k++) {
        if (is_overlapped(
                mem[i]->start, mem[i]->end, mem_info[k]->start,
                mem_info[k]->end)) {
          Static_Info* static_info = get_static_info(info, pc_mem[j]->pc);
          if (static_info->example_line == -1)
            continue;
...
void mark_possible_buffers(
    vector<pc_mem_region_t*>& pc_mem, vector<mem_regions_t*>& mem_regions,
    vector<Static_Info*>& info, vec_cinstr& instrs)
{
  ...
  for (int i = 0; i < mem_regions.size(); i++) {
    ...
    for (int j = 0; j < pc_mem.size(); j++) {
      ...
      for (int k = 0; k < mem_info.size(); k++) {
        if (is_overlapped(
                mem_regions[i]->start, mem_regions[i]->end, mem_info[k]->start,
                mem_info[k]->end)) {
          Static_Info* ind = get_static_info(info, pc_mem[j]->pc);
          if (ind->example_line == -1)
            continue;
          cinstr_t* instr = instrs[ind->example_line].first;
          if (!printed) {
            cout << i << " " << mem_regions[i]->start << " "
                 << get_static_info(info, pc_mem[j]->pc)->disassembly << endl;
            print_cinstr(instr);
    ...
    if (!found) {
      ...
      LOG(log_file, "start : " << mem_regions[i]->start
                               << " end : " << mem_regions[i]->end);
    }
  ...
  for (int i = 0; i < regions.size(); i++) {
    if (regions[i]->start < regions[i]->end && max_addr < regions[i]->end) {
      max_addr = regions[i]->end;
        ...
        regions[i]->start > regions[i]->end && max_addr < regions[i]->start) {
