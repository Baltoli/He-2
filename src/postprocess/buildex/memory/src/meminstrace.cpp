...
void create_mem_layout(
    std::ifstream& in, vector<mem_info_t*>& mem_info, uint32_t version)
{
  ...
  while (!in.eof()) {
    ...
    if (instr != NULL) {
      ...
      for (int i = 0; i < instr->num_srcs; i++) {
        if (instr->srcs[i].type == MEM_HEAP_TYPE
            || instr->srcs[i].type == MEM_STACK_TYPE) {
          ...
          if (instr->srcs[i].type == MEM_STACK_TYPE) {
            ...
            for (int addr = 0; addr < 2; addr++) {
              ...
              if (reg != 0 && reg != DR_REG_EBP && reg != DR_REG_ESP)
                skip = false;
            }
          } else {
            skip = false;
      ...
      for (int i = 0; i < instr->num_dsts; i++) {
        if (instr->dsts[i].type == MEM_HEAP_TYPE
            || instr->dsts[i].type == MEM_STACK_TYPE) {
...
/* only app_pc based pc_mem_region recording is done here - if needed implement
 * the module based recording */
void create_mem_layout(
    std::ifstream& in, vector<pc_mem_region_t*>& mem_info, uint32_t version)
{
...
void create_mem_layout(
    vector<cinstr_t*>& instrs, vector<pc_mem_region_t*>& pc_mems)
{
}
