...
vector<mem_regions_t*> merge_instrace_and_dump_regions(
    vector<mem_regions_t*>& total_regions, vector<mem_info_t*> mem_info,
    vector<mem_regions_t*> mem_regions)
{
  ...
  /* mem regions are from dumps and check whether they overlap with instrace
   * regions */
  for (int i = 0; i < mem_regions.size(); i++) {
    ...
    for (int j = 0; j < mem_info.size(); j++) {
      ...
      if (is_overlapped(
              region_start, region_end - 1, mem_info[j]->start,
              mem_info[j]->end - 1)) {
        ...
        if (debug && debug_level >= 2) {
          ...
          LOG(log_file, "start : " << hex << mem_regions[i]->start
                                   << " end : " << mem_regions[i]->end << endl);
          LOG(log_file, dec << "strides : ");
          for (int k = 0; k < mem_regions[i]->dimensions; k++)
            LOG(log_file, mem_regions[i]->strides[k] << ",");
          LOG(log_file, dec << "extents : ");
          for (int k = 0; k < mem_regions[i]->dimensions; k++)
            LOG(log_file, mem_regions[i]->extents[k] << ",");
          LOG(log_file, endl);
          LOG(log_file, "mem instrace:" << endl);
          LOG(log_file, "start : " << hex << mem_info[j]->start << " end : "
                                   << mem_info[j]->end << dec << endl);
        }

        mem_info_t* info
            = get_the_deepest_enclosing(mem_regions[i], mem_info[j]);
        /* ok if the memory region is completely contained in the region
         * constructed by meminfo */
        if (info != NULL) {
          /* how much to the left of start is the meminfo spread? */
          LOG(log_file, "found deepest enclosing: start "
                            << info->start << " end " << info->end << endl);
          ...
          if (mem_regions[i]->start < mem_regions[i]->end) {
            ...
            for (int k = mem_regions[i]->dimensions - 1; k >= 0; k--) {
              uint32_t spread
                  = (start - info->start) / mem_regions[i]->strides[k];
              start
                  = mem_regions[i]->start - spread * mem_regions[i]->strides[k];
          ...
          if (mem_regions[i]->dimensions == get_number_dimensions(info)) {
            ...
            /* if we get the dimensionality correct on our memory analysis we
               should use it instead of the memory dump information.
            */
            for (int k = 0; k < mem_regions[i]->dimensions; k++) {
              mem_regions[i]->extents[k]
                  = get_extents(info, k + 1, get_number_dimensions(info));
            }
          ...
          if (mem_regions[i]->bytes_per_pixel != info->prob_stride) {
            uint32_t factor
                = info->prob_stride / mem_regions[i]->bytes_per_pixel;
          ...
          DEBUG_PRINT(
              ("WARNING: region is greater than what is accessed; may be not "
               "whole image accessed\n"),
              2);
          LOG(log_file, "WARNING: region is greater than what is accessed; may "
                        "be not whole image accessed\n");
          final_regions.push_back(mem_regions[i]);
          total_regions.push_back(mem_regions[i]);
  ...
  /* create new mem_regions for the remaining mem_info which of type MEM_HEAP -
   * postpone the implementation; these are intermediate nodes */
  for (int i = 0; i < mem_info.size(); i++) {
    if (merged_mem_info[i] == false) { /* if not merged */
      if (mem_info[i]->type = MEM_HEAP_TYPE) {
        ...
        mem->dimensions = get_number_dimensions(
            mem_info[i]); /* we don't know the dimensions of this yet */
...
vector<mem_regions_t*> get_image_regions_from_instrace(
    vector<mem_info_t*>& mem, ifstream& config, image_t* in_image,
    image_t* out_image)
{
...
void mark_regions_type(
    vector<mem_regions_t*> regions, vector<mem_regions_t*> input,
    vec_cinstr& instrs, map<uint32_t, vector<mem_regions_t*>> maps,
  ...
  for (uint32_t i = start; i < end; i++) {
    ...
    if (!input_given) {
      for (int j = 0; j < instr->num_srcs; j++) {
        if ((instr->srcs[j].type == MEM_HEAP_TYPE
             || instr->srcs[j].type == MEM_STACK_TYPE)
            && ((memory_map[instr->srcs[j].value] & WRITTEN) != WRITTEN)) {
          for (int k = 0; k < regions.size(); k++) {
            if (is_overlapped(
                    regions[k]->start, regions[k]->end, instr->srcs[j].value,
                    instr->srcs[j].value)) {
              regions[k]->type |= INPUT_BUFFER;
              memory_map[instr->srcs[j].value] |= READ;
              if (tree->search_node(&instr->srcs[j]) == NULL) {
                tree->add_to_frontier(
                    tree->generate_hash(&instr->srcs[j]),
                    new Conc_Node(&instr->srcs[j]));
              }
            }
          }
        }
    ...
    for (int j = 0; j < amount; j++) {
      if (tree->update_depandancy_forward_with_src(
              &rinstr[j], instr->pc, instrs[i].second->disassembly, i + 1,
              srcs_dep)) {

        for (int k = 0; k < rinstr[j].num_srcs; k++) {
          if ((rinstr[j].srcs[k].type == MEM_STACK_TYPE
               || rinstr[j].srcs[k].type == MEM_HEAP_TYPE)
              && srcs_dep[k]) {
            for (int m = 0; m < regions.size(); m++) {
              if (is_overlapped(
                      regions[m]->start, regions[m]->end, rinstr->srcs[k].value,
                      rinstr->srcs[k].value)) {
                memory_map[instr->srcs[k].value] |= READ;
                if ((regions[m]->type & OUTPUT_BUFFER) == OUTPUT_BUFFER)
                  regions[m]->type |= INTERMEDIATE_BUFFER;
                else
                  regions[m]->type |= INPUT_BUFFER;
        ...
        if ((rinstr[j].dst.type == MEM_STACK_TYPE
             || rinstr[j].dst.type == MEM_HEAP_TYPE)) {
          for (int m = 0; m < regions.size(); m++) {
            if (is_overlapped(
                    regions[m]->start, regions[m]->end, rinstr->dst.value,
                    rinstr->dst.value)) {
...
vector<mem_regions_t*> get_input_output_regions(
    vector<mem_regions_t*>& image_regions,
    vector<mem_regions_t*>& total_regions, vector<pc_mem_region_t*>& pc_mems,
    vector<uint32_t> app_pc, vec_cinstr& instrs, vector<uint32_t> start_points)
{
  ...
  for (int i = 0; i < image_regions.size(); i++) {
    if (image_regions[i]->dump_type == INPUT_BUFFER) {
      input_mem_region = image_regions[i];
      break;
    }
  }
  ...
  for (int i = 0; i < image_regions.size(); i++) {
    if (image_regions[i]->dump_type == OUTPUT_BUFFER) {
      output_mem_region = image_regions[i];
      break;
    }
  }
  ...
  if (input_mem_region == NULL)
    DEBUG_PRINT(("input region missing\n"), 2);
  if (output_mem_region == NULL)
    DEBUG_PRINT(("output region missing\n"), 2);
  ...
  for (int i = 0; i < pc_mems.size(); i++) {
    for (int k = 0; k < total_regions.size(); k++) {
      for (int j = 0; j < pc_mems[i]->regions.size(); j++) {
        if (is_overlapped(
                pc_mems[i]->regions[j]->start, pc_mems[i]->regions[j]->end,
                total_regions[k]->start, total_regions[k]->end)) {
  ...
  if (input_mem_region != NULL) {
    ...
    for (int i = 0; i < start_points.size(); i++) {
      if (i != start_points.size() - 1)
        end = start_points[i + 1];
      else
        end = instrs.size();
      mark_regions_type(
          total_regions, input_regions, instrs, region_map, start, end);
      start = end;
    }
  }
...
vector<mem_regions_t*> get_input_regions(
    vector<mem_regions_t*> total_regions, vector<pc_mem_region_t*>& pc_mems,
    vector<uint32_t> start_points, vec_cinstr& instrs)
{
  ...
  for (int i = 0; i < total_regions.size(); i++) {
    ...
    if ((!total_regions[i]->dependant)
        && ((total_regions[i]->direction & MEM_INPUT) == MEM_INPUT)) {
      ...
      for (int i = 0; i < start_points.size(); i++) {
        ...
        mark_regions_type(
            total_regions, current, instrs, region_map, start, end);
        start = end;
      }
    }
  }
