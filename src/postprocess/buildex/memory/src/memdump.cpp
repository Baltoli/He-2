...
into a abstracted D-dimensional strucuture
...
mem_regions_t* locate_image_CN2(
    char* values, uint32_t size, uint64_t* start_line, uint64_t* end_line,
    image_t* image)
{
  ...
  for (i = *start_line; i < size; i++) {
    ...
    for (int j = 0; j < image->width; j++) {
      if ((values[i + j] & 0xff)
          != image->image_array[j]) { /* this may need to be changed */
        success = false;
        break;
      }
    }
    ...
    if (success) { /* verify the region actually has the image */
      ...
      if (last == image->width * image->height) {
        ...
        if (gaps.size() == 0) {
          ...
          region->end
              = start_points[start_points.size() - 1] + region->strides[1];
...
mem_regions_t* locate_image_CN2_backward_write(
    char* values, uint32_t size, uint64_t* start, uint64_t* end, image_t* image,
    uint32_t bound)
{
  ...
  for (i = *start; i >= 3 * image_size; i--) {
    ...
    for (int j = image->width - 1 - bound; j >= bound; j--) {
      ...
      if ((values[i - val] & 0xff)
          != image->image_array
                 [(0 * image->width * image->height) + j
                  + bound * image->width]) {
        success = false;
        break;
      }
      if ((values[i - (val + 1)] & 0xff)
          != image->image_array
                 [(1 * image->width * image->height) + j
                  + bound * image->width]) { /* this may need to be changed */
        success = false;
        break;
      }
      if ((values[i - (val + 2)] & 0xff)
          != image->image_array
                 [(2 * image->width * image->height) + j
                  + bound * image->width]) { /* this may need to be changed */
        success = false;
        break;
      }
    ...
    if (success) { /* verify the region actually has the image */
      ...
      for (int j = i; j >= image_size; j--) {
        ...
        for (int k = image->width - 1 - bound; k >= bound; k--) {
          ...
          if ((values[j - (val)] & 0xff)
              != (uint8_t)(image->image_array
                               [(0 * image->width * image->height) + last + k
                                + bound * image->width])) {
            found = false;
            break;
          }
          if ((values[j - (val + 1)] & 0xff)
              != (uint8_t)image->image_array
                     [(1 * image->width * image->height) + last + k
                      + bound * image->width]) {
            found = false;
            break;
          }
          if ((values[j - (val + 2)] & 0xff)
              != (uint8_t)image->image_array
                     [(2 * image->width * image->height) + last + k
                      + bound * image->width]) {
            found = false;
            break;
          }
      ...
      if (last == image->width * (image->height - 2 * bound)) {
        ...
        if (found) {
          ...
          region->start = start_points[0];
          region->end
              = start_points[start_points.size() - 1] - region->strides[2];
          *end = region->end;
          return region;
        }
      }
...
mem_regions_t* locate_image_CN2_backward(
    char* values, uint32_t size, uint64_t* start, uint64_t* end, image_t* image)
{
  ...
  for (i = *start; i >= 3 * image_size; i--) {
    ...
    for (int j = image->width - 1; j >= 0; j--) {
      ...
      if ((values[i - val] & 0xff)
          != image->image_array[(0 * image->width * image->height) + j]) {
        success = false;
        break;
      }
      if ((values[i - (val + 1)] & 0xff)
...
vector<mem_regions_t*> get_image_regions_from_dump(
    vector<string> filenames, string in_image_filename,
    string out_image_filename)
{
  ...
  image_t* out_image
      = populate_imageinfo(open_image(out_image_filename.c_str()));
  ...
  for (int i = 0; i < filenames.size(); i++) {
    ...
    ASSERT_MSG(
        stat(filenames[i].c_str(), &results) == 0,
    ...
    while (found_region) {
      ...
      /* locate the image */
      if (write) {
        mem = locate_image_CN2(
            file_values, results.st_size, &start, &end, out_image);
        if (mem == NULL) {
          // mem = locate_image_CN2_backward(file_values, results.st_size,
          // &start_back, &end_back, in_image);
          mem = locate_image_CN2_backward_write(
              file_values, results.st_size, &start_back, &end_back, out_image,
              1);
        }
      } else {
        mem = locate_image_CN2(
            file_values, results.st_size, &start, &end, in_image);
        if (mem == NULL)
          mem = locate_image_CN2_backward(
              file_values, results.st_size, &start_back, &end_back, in_image);
      }
