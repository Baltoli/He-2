...
void update_jump_conditionals(
    Conc_Tree* tree, vec_cinstr& instrs, uint32_t pos);

/************************************************************************/
/*  Tree building routines                                              */
/************************************************************************/

pair<int32_t, int32_t> get_start_and_end_points(
    vector<uint32_t> start_points, uint64_t dest, uint32_t stride,
    uint32_t start_trace, uint32_t end_trace, vec_cinstr& instrs)
{
  ...
  if (start_trace == FILE_BEGINNING) {
    ...
    for (int i = 0; i < instrs.size(); i++) {
      for (int j = 0; j < instrs[i].first->num_dsts; j++) {
        if (instrs[i].first->dsts[j].value == dest
            && instrs[i].first->dsts[j].width == stride) {
          start = i + 1;
          found = true;
          break;
        }
    ...
    // ASSERT_MSG(found, ("ERROR: dest not found within the instruction
    // trace\n"));
  }
...
void build_conc_tree_helper(
    int start_trace, int end_trace, Conc_Tree* tree, vec_cinstr& instrs,
    vector<mem_regions_t*>& regions, vector<Func_Info_t*>& func_info)
{
  ...
  for (; curpos < end_trace; curpos++) {
    ...
    rinstr = cinstr_to_rinstrs(
        instr, no_rinstrs, instrs[curpos].second->disassembly, curpos);
    if (debug_level >= 4) {
      print_rinstrs(log_file, rinstr, no_rinstrs);
    }

    bool updated = false;
    bool affected = false;
    for (int i = no_rinstrs - 1; i >= 0; i--) {
      updated = tree->update_depandancy_backward(
          &rinstr[i], instrs[curpos].first, instrs[curpos].second, curpos,
          regions, func_info);
      if (updated) {
        affected = true;
...
void build_tree_intial_update(
    uint64_t destination, uint32_t stride, uint32_t start, uint32_t end,
    Conc_Tree* tree, vec_cinstr& instrs, uint32_t original_start,
    vector<mem_regions_t*>& regions, vector<Func_Info_t*>& func_info)
{
  ...
  for (int i = end - 1; i >= start; i--) {
    ...
    for (int j = 0; j < instr->num_dsts; j++) {
      ...
      if (is_overlapped(
              destination, destination + stride - 1, opnd.value,
              opnd.value + opnd.width - 1)) {
        rinstr = cinstr_to_rinstrs_eflags(
            instr, amount, instrs[i].second->disassembly, i);
        for (int k = amount - 1; k >= 0; k--) {
          if (rinstr[k].dst.value == opnd.value
              && rinstr[k].dst.width == opnd.width) {
            found = true;
            index = k;
            break;
          }
  ...
  ASSERT_MSG(
      (index != -1), ("ERROR: could not find the destination or overlap\n"));

  for (int i = index; i >= 0; i--) {
    tree->update_depandancy_backward(
        &rinstr[i], instr, instrs[curpos].second, curpos, regions, func_info);
  }
  delete[] rinstr;
  ...
  build_conc_tree_helper(
      start_trace, end_trace, tree, instrs, regions, func_info);
...
void build_dummy_initial_tree(
    Conc_Tree* initial_tree, Conc_Tree* red_tree, mem_regions_t* new_region)
{
  ...
  ASSERT_MSG(
      (region != NULL), ("ERROR: the head should have an output region\n"));
  ...
  new_node->symbol->value
      = node->symbol->value - region->start + new_region->start;
  new_node->region = new_region;

  dup_node->add_forward_ref(new_node);
}

Conc_Tree* build_conc_tree(
    uint64_t destination, uint32_t stride, std::vector<uint32_t> start_points,
    int start_trace, int end_trace, Conc_Tree* tree, vec_cinstr& instrs,
    uint64_t farthest, vector<mem_regions_t*>& regions,
    vector<Func_Info_t*>& func_info)
{
  ...
  pair<int32_t, int32_t> points = get_start_and_end_points(
      start_points, destination, stride, start_trace, end_trace, instrs);
  ...
  if (end_trace != FILE_ENDING) {
    ASSERT_MSG(
        ...
        ("ERROR: trace end should be greater than the trace start\n"));
  } else {
  ...
  if (start_trace != FILE_BEGINNING) {
    ASSERT_MSG(
        (start_trace < instrs.size()),
        ("ERROR: trace start should be within the limits of the file"));
  ...
  // major assumption here is that reg and mem 'value' fields do not overlap.
  // This is assumed in all other places as well. can have an assert for this
  instr = instrs[curpos].first;
  DEBUG_PRINT(
      ("starting from instr - %s\n",
       instrs[curpos].second->disassembly.c_str()),
      3);
  rinstr = cinstr_to_rinstrs(
      instr, no_rinstrs, instrs[curpos].second->disassembly, curpos);
  ...
  for (int i = no_rinstrs - 1; i >= 0; i--) {
    if (rinstr[i].dst.value == destination) {
      ASSERT_MSG(
          (rinstr[i].dst.type != IMM_FLOAT_TYPE)
              && (rinstr[i].dst.type != IMM_INT_TYPE),
          ("ERROR: dest cannot be an immediate\n"));
      index = i;
      dest_present = true;
      break;
    }
  ...
  if (dest_present == false || index < 0)
    return NULL;

  ASSERT_MSG(
      (dest_present == true) && (index >= 0),
      ("ERROR: couldn't find the dest to start trace\n")); // we should have
                                                           // found the
                                                           // destination
  ...
  for (int i = 0; i < start_points.size(); i++) {
    if (start_points[i] == end_trace)
      index = i;
  }

  for (int i = start_points.size() - 1; i >= 0; i--) {
    if (start_points[i] < start_to_initial) {
      start_to_initial = start_points[i];
      break;
    }
  }

  if (start_to_initial == start_trace)
    start_to_initial = 0;
  ...
  if (tree->recursive) {
    ...
    build_tree_intial_update(
        destination, stride, start_to_initial, end_trace, initial_tree, instrs,
        initial_start, regions, func_info);

    if (initial_tree->get_head() == NULL) {
      /* ok, there is no initial first create a new region if the dummy region
       * is not built */
      ...
      if (region == NULL) {
        ...
        region->end
            = conc_node->region->end - conc_node->region->start + farthest;
  ...
  if (debug_tree) {
    ...
    ofstream file(
        get_standard_folder("output") + "\\tree_" + to_string(destination)
        + ".dot");
    tree->number_tree_nodes();
    ...
    ofstream abs_file(
        get_standard_folder("output") + "\\abs_tree_" + to_string(destination)
        + ".dot");
...
void build_conc_tree_single_func(
    uint64_t destination, uint32_t stride, std::vector<uint32_t> start_points,
    int start_trace, int end_trace, Conc_Tree* tree, vec_cinstr& instrs,
    vector<mem_regions_t*>& regions, vector<Func_Info_t*>& func_info)
{
  ...
  ASSERT_MSG(
      (instr != NULL), ("ERROR: you have given a line no beyond this file\n"));
  ...
  for (int i = index; i >= 0; i--) {
    if (instrs[curpos].second != NULL) {
      ...
      tree->update_depandancy_backward(
          &rinstr[i], instrs[curpos].first, NULL, curpos, regions, func_info);
    }
  }
    ...
    if (instr != NULL) {
      ...
      for (int i = no_rinstrs - 1; i >= 0; i--) {
        if (instrs[curpos].second != NULL) {
          ...
          updated = tree->update_depandancy_backward(
              &rinstr[i], instrs[curpos].first, NULL, curpos, regions,
              func_info);
        }
...
void update_jump_conditionals(Conc_Tree* tree, vec_cinstr& instrs, uint32_t pos)
{

  cinstr_t* instr = instrs[pos].first;
  Static_Info* static_info = instrs[pos].second;
  vector<pair<Jump_Info*, bool>> input_dep_conditionals
      = static_info->conditionals;

  DEBUG_PRINT(
      ("checking for conditionals attached - instr %d\n", instr->pc), 4);
  ...
  for (int i = 0; i < input_dep_conditionals.size(); i++) {
    ...
    ASSERT_MSG(
        (line_cond != 0),
        ("ERROR: couldn't find the conditional instruction\n"));
    ASSERT_MSG(
        (line_jump != 0), ("ERROR: couldn't find the jump instruction\n"));

    // added
    bool actual_taken = is_branch_taken(
        instrs[line_jump].first->opcode, instrs[line_jump].first->eflags);

    if (jump_info->target_pc != jump_info->fall_pc) {
      ASSERT_MSG(
          (actual_taken == taken),
          ("ERROR: branch direction information is inconsistent\n"));
...
void build_conc_tree(
    uint64_t destination, uint32_t stride, std::vector<uint32_t> start_points,
    int start_trace, int end_trace, Conc_Tree* tree, std::ifstream& file,
...
void build_conc_trees_for_conditionals(
    std::vector<uint32_t> start_points, Conc_Tree* tree, vec_cinstr& instrs,
    uint64_t farthest, vector<mem_regions_t*>& regions,
    vector<Func_Info_t*>& func_info)
{
  ...
  for (int i = 0; i < tree->conditionals.size(); i++) {
    ...
    for (int j = 0; j < instr->num_srcs; j++) {
      ...
      if (instr->srcs[j].type != IMM_INT_TYPE
          && instr->srcs[j].type != IMM_FLOAT_TYPE) {
        ...
        for (int k = line_cond; k < instrs.size(); k++) {
          ...
          for (int m = 0; m < temp->num_dsts; m++) {
            if (temp->dsts[m].type == instr->srcs[j].type
                && temp->dsts[m].value == instr->srcs[j].value) {
              dst_line = k;
              found = true;
              break;
            }
        ...
        ASSERT_MSG(
            (dst_line != 0),
            ("ERROR: couldn't find the conditional destination\n"));

        Conc_Tree* cond_tree = new Conc_Tree();
        build_conc_tree(
            instr->srcs[j].value, instr->srcs[j].width, start_points,
            dst_line + 1, FILE_ENDING, cond_tree, instrs, farthest, regions,
            func_info);
        ...
        if (instr->srcs[j].type == IMM_INT_TYPE) {
          head = new Conc_Node(
              instr->srcs[j].type, instr->srcs[j].value, instr->srcs[j].width,
              0.0);
        } else {
          head = new Conc_Node(
              instr->srcs[j].type, 0, instr->srcs[j].width,
              instr->srcs[j].float_value);
        }
    ...
    Node* new_head_node = new Conc_Node(
        comp_node->symbol->type, comp_node->symbol->value,
        comp_node->symbol->width, 0.0);
    ...
    if (instr->opcode == OP_cmp) {
      ...
      node->operation
          = dr_logical_to_operation(instrs[line_jump].first->opcode);
      ...
      create_dependancy(
          head_node,
          new Conc_Node(IMM_INT_TYPE, 0, 4, 0.0)); /* immediate zero node */
      ...
      ASSERT_MSG(
          (false),
          ("ERROR: conditionals for other instructions are not done - %s\n",
...
std::vector<Conc_Tree*> get_similar_trees(
    ...
    std::vector<mem_regions_t*>& total_regions, uint32_t seed, uint32_t* stride,
    std::vector<uint32_t> start_points, int32_t start_trace, int32_t end_trace,
    uint64_t farthest, vec_cinstr& instrs, vector<Func_Info_t*>& func_info)
{
  ...
  *stride = random_mem_region->bytes_per_pixel;
  ...
  Conc_Tree* initial_tree = build_conc_tree(
      mem_location, *stride, start_points, FILE_BEGINNING, end_trace, main_tree,
      instrs, farthest, total_regions, func_info);
  nodes.push_back(main_tree);
  if (initial_tree != NULL)
    nodes.push_back(initial_tree);
  ...
  while (true) {
    ...
    while (true) {
      ...
      for (int j = 0; j < base.size(); j++) {
        if (j == affected_dim) {
          offset.push_back(sign);
        } else {
          offset.push_back(next_value++);
        }
      }
      ...
      bool success;
      mem_location
          = get_mem_location(base, offset, random_mem_region, &success);

      if (success) {
        Conc_Tree* created_tree = new Conc_Tree();
        initial_tree = build_conc_tree(
            mem_location, random_mem_region->bytes_per_pixel, start_points,
            FILE_BEGINNING, end_trace, created_tree, instrs, farthest,
            total_regions, func_info);
        ...
        if (main_tree->are_trees_similar(created_tree)) {
          ...
          dims[affected_dim]++;
          sign = -1 * sign
                 * (int)ceil((i + 1) / (double)random_mem_region->dimensions);
...
std::vector<std::vector<Conc_Tree*>> cluster_trees(
    ...
    std::vector<uint32_t> start_points, vec_cinstr& instrs, uint64_t farthest,
    std::string output_folder, std::vector<Func_Info_t*>& func_info)
{

  DEBUG_PRINT(
      ("Building trees for all locations in the output and clustering\n"), 2);
  ...
  while (!done) {
    ...
    tree->tree_num = i;
    Conc_Tree* initial_tree = build_conc_tree(
        location, mem->bytes_per_pixel, start_points, FILE_BEGINNING,
        FILE_ENDING, tree, instrs, farthest, total_regions, func_info);
    build_conc_trees_for_conditionals(
        start_points, tree, instrs, farthest, total_regions, func_info);
    trees.push_back(tree);
    if (initial_tree != NULL) {
      build_conc_trees_for_conditionals(
          start_points, initial_tree, instrs, farthest, total_regions,
          func_info);
    ...
    if (count == indexes.size() / fraction)
      done = true;
  }
  DEBUG_PRINT(("\n"), 2);
  ...
  mem->start), 2); Conc_Tree * tree = new Conc_Tree(); tree->tree_num = i -
  mem->start; Conc_Tree * initial_tree = build_conc_tree(i,
  mem->bytes_per_pixel, start_points, FILE_BEGINNING, FILE_ENDING, tree, instrs,
  farthest, total_regions); build_conc_trees_for_conditionals(start_points,
  tree, instrs, farthest, total_regions); trees.push_back(tree); if
  (initial_tree != NULL){ build_conc_trees_for_conditionals(start_points,
  initial_tree, instrs, farthest, total_regions); trees.push_back(initial_tree);
          }
  ...
  if (clustered_trees.size()
      == trees.size()) { /* so each tree is a seperate cluster; assume that all
                            the trees are in a single cluster and this would
                            fail is similarity checking in abstrees */
  ...
  ofstream conc_file(output_folder + "_conctree_" + to_string(i) + ".dot",
  ofstream::out); print_to_dotfile(conc_file, clustered_trees[i][1]->get_head(),
  no_nodes, 0);
  }*/
...
vector<Tree*> get_linearly_independant_trees(
    vector<Conc_Tree*> cluster, vector<mem_regions_t*> regions)
{

  vector<Tree*> ret;
  mem_regions_t* region
      = get_mem_region(cluster[0]->get_head()->symbol->value, regions);
  vector<int32_t> pos;

  for (int i = 0; i < cluster.size(); i++) {
    pos = get_mem_position(
        get_mem_region(cluster[i]->get_head()->symbol->value, regions),
        cluster[i]->get_head()->symbol->value);
    if (pos[0] != 0 && pos[0] != -1) {
      ret.push_back(cluster[i]);
      break;
    }
  }
  ...
  for (int i = 0; i < pos.size(); i++) {
    ...
    while (!found && j >= 0 && j < region->extents[i]) {
      ...
      if (now_regions[i] == next[i])
        continue;

      cout << "regions " << now_regions[i] << endl;
      for (int k = 0; k < cluster.size(); k++) {
        vector<int32_t> now
            = get_mem_position(region, cluster[k]->get_head()->symbol->value);
        if (now_regions == now) {
          ret.push_back(cluster[k]);
          done = true;
          found = true;
  ...
  ASSERT_MSG(
      ret.size() > region->dimensions, ("ERROR: not enought equations\n"));

  return ret;
}

Abs_Tree* abstract_the_trees(
    vector<Conc_Tree*> cluster, uint32_t no_trees, uint32_t skip,
    vector<mem_regions_t*>& total_regions, vector<pc_mem_region_t*>& pc_mem)
{
  ...
  // vector<Tree *> ind_trees = get_linearly_independant_trees(cluster,
  // total_regions);
  int count = 0;
  srand(4);
  ...
  /*for (int j = 0; j < ind_trees.size(); j++){
          Abs_Tree  * abs_tree = new Abs_Tree();
          //identify_parameters(cluster[j]->head, pc_mem);
          abs_tree->build_abs_tree_unrolled((Conc_Tree *)ind_trees[j],
  total_regions);
  ...
  if (similar) {
    ...
    DEBUG_PRINT(
        ("the trees are similar, now getting the algebric filter....\n"), 1);
    ...
    for (int i = 0; i < trees.size(); i++) {
      ofstream file(
          get_standard_folder("output") + "\\abs_tree_not_sim" + to_string(i)
          + ".dot");
...
vector<pair<Abs_Tree*, bool>> get_conditional_trees(
    vector<Conc_Tree*> clusters, uint32_t no_trees,
    vector<mem_regions_t*> total_regions, uint32_t skip,
    vector<pc_mem_region_t*>& pc_mem)
{
  ...
  for (int i = 1; i < no_trees; i++) {
    ...
    ASSERT_MSG(
        (tree->conditionals.size() == number_conditionals),
        ("ERROR: number of conditionals in similar trees different\n"));
  }
  ...
  for (int i = 0; i < clusters[0]->conditionals.size(); i++) {
    ...
    for (int k = 0; k < skip * no_trees; k += skip) {
      /* add the computational tree head (computational node should have been
       * already put out) */
      trees.push_back(clusters[k]->conditionals[i]->tree);
    }

    Abs_Tree* cond_abs_tree
        = abstract_the_trees(trees, no_trees, 1, total_regions, pc_mem);

    cond_abs_trees.push_back(
        make_pair(cond_abs_tree, clusters[0]->conditionals[i]->taken));
  }
...
vector<Abs_Tree_Charac*> build_abs_trees(
    std::vector<std::vector<Conc_Tree*>> clusters, std::string folder,
    uint32_t no_trees, std::vector<mem_regions_t*> total_regions, uint32_t skip,
    std::vector<pc_mem_region_t*>& pc_mem)
{
  ...
  for (int i = 0; i < clusters.size(); i++) {
    ...
    mem_regions_t* cluster_region = get_mem_region(
        clusters[i][0]->get_head()->symbol->value, total_regions);
    for (int j = 0; j < clusters[i].size(); j++) {
      vector<int32_t> pos = get_mem_position(
          cluster_region, clusters[i][j]->get_head()->symbol->value);
    ...
    ofstream conc_file(
        folder + "\\conc_comp_" + to_string(i) + ".dot", ofstream::out);
    ...
    for (int j = 0; j < tree->conditionals.size(); j++) {
      ...
      ofstream conc_file(
          folder + "\\conc_cond_" + to_string(i) + "_" + to_string(j) + ".dot",
          ofstream::out);
  ...
  for (int i = 0; i < clusters.size(); i++) {
    ...
    Abs_Tree* abs_tree = abstract_the_trees(
        clusters[i], no_trees, skip_trees, total_regions, pc_mem);
    /* get the conditional trees*/
    abs_tree->conditional_trees = get_conditional_trees(
        clusters[i], no_trees, total_regions, skip_trees, pc_mem);

    ofstream abs_file(
        folder + "\\symbolic_tree_" + to_string(i) + ".dot", ofstream::out);
    uint32_t max_dimensions = abs_tree->get_maximum_dimensions();
    abs_tree->number_tree_nodes();
    abs_tree->print_dot_algebraic(
        abs_file, "alg", 0, get_vars("x", max_dimensions));

    /* check if the trees are recursive*/
    if (abs_tree
            ->recursive) { /* if yes, then try to get the reduction domain */
      ...
      if (indirect_node != NULL) {
        Abs_Node* abs_node
            = abs_tree->find_indirect_node((Abs_Node*)indirect_node);
        ...
        last_tree->build_abs_tree_unrolled(
            clusters[i][clusters[i].size() - 1], total_regions);
        ...
        for (int j = 0; j < first_head->mem_info.dimensions; j++) {
          DEBUG_PRINT(
              ("%d %d\n", first_head->mem_info.pos[j],
               last_head->mem_info.pos[j]),
              2);
        }
        ...
        if (!gap) { /* get the abs pos of the first and last trees */
          // DEBUG_PRINT(("no gaps\n"), 2);
          for (int j = 0; j < first_head->mem_info.dimensions; j++) {
            charac->extents.push_back(make_pair(
                first_head->mem_info.pos[j], last_head->mem_info.pos[j]));
          }
          charac->gaps_in_rdom = false;
        } else {
          ...
          for (int j = 0; j < first_head->mem_info.dimensions; j++) {
            charac->extents.push_back(make_pair(
                0, first_head->mem_info.associated_mem->extents[j] - 1));
          }
        }
      }
  ...
  /* now let's check for recursive trees whether there are opportunities with
   * dummy trees - this is to avoid multi-image solving; in reality this part
   * can be removed */
    ...
    if (charac->is_recursive && charac->red_node == NULL) {
      ...
      if (size == rdom_size) {
        ...
        for (int j = 0; j < tree_characs.size(); j++) {
          ...
          if (!now_charac->is_recursive
              && (now_node->mem_info.associated_mem
                  == node->mem_info.associated_mem)
              && now_charac->tree->dummy_tree) {
