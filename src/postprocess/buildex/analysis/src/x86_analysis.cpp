...
#define elseif_bounds(d, s)                                                    \
  else if ((cinstr->num_dsts == d) && (cinstr->num_srcs == s))
#define else_bounds                                                            \
  else { unhandled = true; }                                                   \
  break

/* function prototypes */
static void populate_rinstr(
    rinstr_t* rinstr, operand_t dst, int num_srcs, operand_t src1,
    operand_t src2, int operation, bool sign);
bool check_eflag_bit(eflag_bits flag_type, uint32_t reg_val);
bool check_lahf_bit(lahf_bits flag_type, uint32_t reg_val);
static void assert_opnds(
    int opcode, int needed_src, int needed_dst, int actual_src, int actual_dst);
...
This function is similar to cinstr_to_rinstrs in all ways except in addition, it
canonicalizes instructions which affect eflags but does not write to any
destination permanently.

*/
rinstr_t* cinstr_to_rinstrs_eflags(
    cinstr_t* cinstr, int& amount, std::string disasm, uint32_t line)
{

  // get already handled instructions which pose true dependencies - this will
  // decide whether an instruction is not handled at all
  ...
  if (rinstr == NULL) {
    ...
    case OP_cmp:
      if_bounds(0, 2)
      {
        ...
        amount = 1;
        operand_t virtual_reg
            = {REG_TYPE, cinstr->srcs[0].width, DR_REG_VIRTUAL_1};
        reg_to_mem_range(&virtual_reg);
        rinstr[0] = {
            op_sub, virtual_reg, 2, {cinstr->srcs[0], cinstr->srcs[1]}, true};
      }
      else_bounds;
    case OP_test:
      if_bounds(0, 2)
      {
        ...
        reg_to_mem_range(&virtual_reg);
        rinstr[0] = {
            op_and, virtual_reg, 2, {cinstr->srcs[0], cinstr->srcs[1]}, true};
      }
      else_bounds;
    }
    ...
    ASSERT_MSG(
        (!unhandled),
        ("ERROR: opcode %s(%d) with %d dests and %d srcs (app_pc - %d) not "
         "handled in canonicalization\n",
...
Parameters
 ...
 amount - pass by reference variable, which will contain how many reduced set
instructions were need to canonicalize the given x86 instruction disasm - string
of the disassembly of 'cinstr'. Can be used for debuggin purposes line	- the
line in which this instruction can be found in the instruction trace (after
filtering)
    ...
    cinstr_t* cinstr, int& amount, std::string disasm, uint32_t line)
{
  ...
  case OP_movss:
    ...
    if_bounds(1, 1)
    {
      // move low 32 bits from src to dst. Upper bits unchanged if dst is
      // register bits 127-32 zeroed if source is memory
  ...
  case OP_pmuldq:
    ...
    if_bounds(2, 1)
    {
      ...
      // create virtual reg/mem accessing 32 bit input ranges tmp 64 bit reg and
      // 2 tmp 32 bit reg
      operand_t input1 = cinstr->srcs[0];
      operand_t input2 = cinstr->srcs[1];
      operand_t output = cinstr->dsts[0];
      // offset starting point by correct number of BYTES (divide by 8 to change
      // from bits to bytes)
      operand_t input32_1
          = {input1.type, 4, input1.value + input1.width - 4, NULL};
      operand_t input32_2
          = {input2.type, 4, input2.value + input2.width - 4, NULL};
      operand_t input96_1
          = {input1.type, 4, input1.value + input1.width - 12, NULL};
      operand_t input96_2
          = {input2.type, 4, input2.value + input2.width - 12, NULL};
      // create virtual reg/mem accessing 8 byte output ranges
      operand_t output_low
          = {output.type, 8, output.value + output.width - 8, NULL};
      operand_t output_high
          = {output.type, 8, output.value + output.width - 16, NULL};
  ...
  case OP_imul:
    ...
    if_bounds(1, 2)
    {
      ...
      amount = 1;
      rinstr[0] = {
          op_mul, cinstr->dsts[0], 2, {cinstr->srcs[0], cinstr->srcs[1]}, true};
    }
    elseif_bounds(2, 2)
    {
      ...
      operand_t virtual_reg
          = {REG_TYPE, 2 * cinstr->srcs[1].width, DR_REG_VIRTUAL_1, NULL};
      reg_to_mem_range(&virtual_reg);

      // virtual <= eax * src0
      rinstr[0]
          = {op_mul, virtual_reg, 2, {cinstr->srcs[1], cinstr->srcs[0]}, true};
  ...
  case OP_mul:
    ...
    if_bounds(2, 2)
    {
      ...
      // virtual <= eax * src0
      rinstr[0]
          = {op_mul, virtual_reg, 2, {cinstr->srcs[1], cinstr->srcs[0]}, false};
  ...
  case OP_idiv:
    ...
    if_bounds(2, 3)
    {
      ...
      // virtual <- edx:eax
      rinstr[0] = {
          op_concat, virtual_reg, 2, {cinstr->srcs[1], cinstr->srcs[2]}, false};

      // edx <- virtual % src[0]
      rinstr[1]
          = {op_mod, cinstr->dsts[0], 2, {virtual_reg, cinstr->srcs[0]}, true};

      // eax <- virtual / src[0]
      rinstr[2]
          = {op_div, cinstr->dsts[1], 2, {virtual_reg, cinstr->srcs[0]}, true};
    }
    else_bounds;
  ...
  case OP_xchg:
    ...
    if_bounds(2, 2)
    {
      ...
      amount = 3;
      operand_t virtual_reg
          = {REG_TYPE, cinstr->srcs[0].width, DR_REG_VIRTUAL_1, NULL};
      reg_to_mem_range(&virtual_reg);

      ASSERT_MSG(
          ((cinstr->dsts[0].value == cinstr->srcs[0].value)
           && (cinstr->dsts[1].value == cinstr->srcs[1].value)),
          ("op_fxch the dsts and srcs should match\n"));
    ...
    if_bounds(1, 2)
    {
      ...
      rinstr[0]
          = {operation,
             ...
             2,
             {cinstr->srcs[1], cinstr->srcs[0]},
             false}; /* changed for SUB (src1, src0) from the reverse: please
                        verify */
    }
    else_bounds;
  ...
  case OP_dec:
    if_bounds(1, 1)
    {
      ...
      rinstr[0]
          = {op_sub, cinstr->dsts[0], 2, {cinstr->srcs[0], immediate}, true};
    }
    else_bounds;
  ...
  case OP_inc:
    if_bounds(1, 1)
    {
      ...
      operand_t immediate = {IMM_INT_TYPE, cinstr->srcs[0].width, 1, NULL};
      rinstr[0]
          = {op_add, cinstr->dsts[0], 2, {cinstr->srcs[0], immediate}, true};
    }
    else_bounds;
  ...
  case OP_sar:
    ...
    if_bounds(1, 2)
    {
      ...
      amount = 1;
      rinstr[0] = {
          op_rsh, cinstr->dsts[0], 2, {cinstr->srcs[1], cinstr->srcs[0]}, true};
    }
    else_bounds;
  ...
  case OP_lea:
    ...
    if_bounds(1, 4)
    {
      ...
      if (cinstr->srcs[2].value == 0) {
        ...
        rinstr[0] = {
            op_mul, virtual_reg, 2, {cinstr->srcs[2], cinstr->srcs[1]}, true};
        // virtual <= virtual + base(src0)
        rinstr[1]
            = {op_add, virtual_reg, 2, {virtual_reg, cinstr->srcs[0]}, true};
        // dst <= virtual + disp
        rinstr[2] = {
            op_add, cinstr->dsts[0], 2, {virtual_reg, cinstr->srcs[3]}, true};
      }
    }
    else_bounds;
  ...
  case OP_sbb:
    ...
    if_bounds(1, 2)
    {
      ...
      if ((cinstr->srcs[0].type == cinstr->srcs[1].type)
          && (cinstr->srcs[0].value == cinstr->srcs[1].value)) {
        ...
        rinstr[0]
            = {op_sub,
               ...
               2,
               {cinstr->srcs[1], cinstr->srcs[0]},
               true}; /* changed for SUB (src1, src0) from the reverse: please
                         verify */
      }
      if (cf) { // substract an immediate 1
        ...
        rinstr[1] = {
            op_sub, cinstr->dsts[0], 2, {cinstr->dsts[0], immediate_1}, true};
      }
    }
    else_bounds;
  ...
  case OP_adc:
    ...
    if_bounds(1, 2)
    {
      ...
      // dsts[0] <- srcs[1] + srcs[0]
      rinstr[0] = {
          op_add, cinstr->dsts[0], 2, {cinstr->srcs[1], cinstr->srcs[0]}, true};
      // dsts[0] <- dsts[0] + 1
      if (cf) {
        operand_t immediate_1 = {IMM_INT_TYPE, cinstr->srcs[1].width, 1, NULL};
        rinstr[1] = {
            op_add, cinstr->dsts[0], 2, {cinstr->dsts[0], immediate_1}, true};
      }
    }
    else_bounds;
  ...
  case OP_cmovnz:
    if_bounds(1, 2)
    {
      ...
      bool check = false;
      if (cinstr->opcode == OP_cmovl)
        check = sf != of;
      else if (cinstr->opcode == OP_cmovle)
        check = zf || (sf != of);
      else if (cinstr->opcode == OP_cmovnle)
        check = !zf || (sf == of);
      else if (cinstr->opcode == OP_cmovnl)
        check = sf == of;
      else if (cinstr->opcode == OP_cmovz)
        check = zf;
      else if (cinstr->opcode == OP_cmovnz)
        check = !zf;
      else if (cinstr->opcode == OP_cmovns)
        check = !sf;
  ...
  case OP_setb:
    ...
    if_bounds(1, 0)
    {
      ...
      switch (cinstr->opcode) {
      case OP_setz:
        flag = check_lahf_bit(Zero_lahf, cinstr->eflags);
        break;
      case OP_setnz:
        flag = check_lahf_bit(Zero_lahf, cinstr->eflags);
        break;
      case OP_sets:
        flag = check_lahf_bit(Sign_lahf, cinstr->eflags);
        break;
      case OP_setns:
        flag = !check_lahf_bit(Sign_lahf, cinstr->eflags);
        break;
      case OP_setb:
        flag = check_lahf_bit(Carry_lahf, cinstr->eflags);
        break;
      }
      if (flag) {
  ...
  case OP_xadd:
    ...
    if_bounds(2, 2)
    {
      ...
      reg_to_mem_range(&virtual_reg);
      rinstr[0]
          = {op_add, virtual_reg, 2, {cinstr->srcs[0], cinstr->srcs[1]}, true};
  ...
  case OP_fmulp: // Multiply ST(i) by ST(0), store result in ST(i), and pop the
                 // register stack.
    if_bounds(1, 2)
    {
      rinstr = new rinstr_t[1];
  ...
  case OP_faddp: // Add ST(0) to ST(i), store result in ST(i), and pop the
                 // register stack
  case OP_fadd: // Add m32fp to ST(0) and store result in ST(0).
  case OP_fsubp: // Subtract ST(0) from ST(1), store result in ST(1), and pop
                 // register stack.
  case OP_fsub: // Subtract m32fp from ST(0) and store result in ST(0).
  case OP_fdivp: // Divide ST(1) by ST(0), store result in ST(1), and pop the
                 // register stack.
...
void cinstr_convert_reg(cinstr_t* instr)
{
  ...
  for (int i = 0; i < instr->num_srcs; i++) {
    if ((instr->srcs[i].type == REG_TYPE)
        && (instr->srcs[i].value > DR_REG_ST7))
      instr->srcs[i].value += 8;
    reg_to_mem_range(&instr->srcs[i]);
  }

  for (int i = 0; i < instr->num_dsts; i++) {
    if (instr->dsts[i].type == REG_TYPE && (instr->dsts[i].value > DR_REG_ST7))
      instr->dsts[i].value += 8;
...
void reg_to_mem_range(operand_t* opnd)
{
  ...
  if (opnd->type == REG_TYPE) {
    ...
    printf("after %s\n", opnd_to_string(opnd).c_str());
      ...
      ((opnd->type == MEM_HEAP_TYPE) || (opnd->type == MEM_STACK_TYPE))
      && (opnd->width != 0)) {
...
int mem_range_to_reg(operand_t* opnd)
{
  ...
  if (opnd->type == REG_TYPE) {
    ...
    case 55:
      return DR_REG_VIRTUAL_1;
    case 56:
      return DR_REG_VIRTUAL_2;
...
void update_regs_to_mem_range(vec_cinstr& instrs)
{
  ...
  for (int i = 0; i < instrs.size(); i++) {
    ...
    for (int i = 0; i < instr->num_srcs; i++) {
      ...
      if (instr->srcs[i].addr != NULL) {
        for (int j = 0; j < 4; j++) {
          if (instr->srcs[i].addr[j].type == REG_TYPE
              && instr->srcs[i].addr[j].value == 0) {
            continue;
          }
    ...
    for (int i = 0; i < instr->num_dsts; i++) {
      if (instr->dsts[i].type == REG_TYPE
          && (instr->dsts[i].value > DR_REG_ST7))
        instr->dsts[i].value += 8;
      reg_to_mem_range(&instr->dsts[i]);
      if (instr->dsts[i].addr != NULL) {
        for (int j = 0; j < 4; j++) {
          if (instr->dsts[i].addr[j].type == REG_TYPE
              && instr->dsts[i].addr[j].value == 0) {
            continue;
          }
...
bool is_floating_point_reg(operand_t* opnd)
{
  ...
  return (
      (opnd->type == REG_TYPE) && (reg >= DR_REG_ST0) && (reg <= DR_REG_ST7));
}
...
void get_floating_point_reg(operand_t* opnd, string disasm, uint32_t line)
{
  ...
  // cout << line << " floating point reg : " << regname_to_string(ret)  << " "
  // << disasm << endl; cout << "tos_f: " << tos << endl;
  ASSERT_MSG(
      (ret <= (int32_t)DR_REG_ST15) && (ret >= (int32_t)DR_REG_ST0),
      ("ERROR: floating point stack under/overflow"));
...
void update_tos(uint32_t type, string disasm, uint32_t line, uint32_t direction)
{
  ...
  if (direction == BACKWARD_ANALYSIS) {
    if (type == FP_PUSH) {
      ...
      ASSERT_MSG(
          tos < DR_REG_ST15, ("ERROR: floating point stack underflow\n"));
      tos++;
    }
...
void update_floating_point_regs(
    vec_cinstr& instrs, uint32_t direction, vector<Static_Info*> static_info,
    vector<uint32_t> pc)
{
  ...
  for (int i = 0; i < instrs.size(); i++) {
    ...
    if (!is_floating_point_ins(cinstr->opcode)) {
      ...
      case OP_fistp: // Store ST(0) in m32int and pop register stack.
        ...
        break;
      case OP_fmul: // Multiply ST(0) by m32fp and store result in ST(0).
      case OP_fmulp: // Multiply ST(i) by ST(0), store result in ST(i), and pop
                     // the register stack.
      ...
      case OP_faddp: // Add ST(0) to ST(i), store result in ST(i), and pop the
                     // register stack
      case OP_fadd: // Add m32fp to ST(0) and store result in ST(0).
      case OP_fsubp: // Subtract ST(0) from ST(1), store result in ST(1), and
                     // pop register stack.
      case OP_fsub: // Subtract m32fp from ST(0) and store result in ST(0).
      case OP_fdivp: // Divide ST(1) by ST(0), store result in ST(1), and pop
                     // the register stack.
      case OP_fdiv: // Divide ST(0) by m32fp and store result in ST(0).
        if (((cinstr->opcode == OP_faddp) || (cinstr->opcode == OP_fsubp)
             || (cinstr->opcode == OP_fdivp))
            && (direction == BACKWARD_ANALYSIS)) {
          update_tos(FP_POP, disasm, line, direction);
        }
        update_fp_reg(cinstr, disasm, line);
        if (((cinstr->opcode == OP_faddp) || (cinstr->opcode == OP_fsubp)
             || (cinstr->opcode == OP_fdivp))
            && (direction == FORWARD_ANALYSIS)) {
          update_tos(FP_POP, disasm, line, direction);
        }
        break;
      case OP_fcomp:
...
/* this returns the flag mask register of the affected flags by the relevant
opcode Function - is_eflags_affected

Parameters
 opcode - opcode of the x86 instruction

Return
 uint32_t - the mask register which carries a bit vector of which flags are
affected by which instruction
...
uint32_t is_eflags_affected(uint32_t opcode)
{
  ...
  case OP_shr:
  case OP_shl:
    // OF, CF, SF, ZF, and PF (AF is generally undefined) - there are conditions
    // for OF, CF as well but we will disregard them for now
  case OP_xor:
  case OP_and:
  case OP_or:
...
Parameters
 opcode - x86 instruction opcode, make sure this instruction is a conditional
jump flags  - the actual value of the flags recovered by DR instrumentation at
runtime via lahf instruction (refer lahf_bits to find the exact position)

Return
 bool - returns, based on the jump instruction and the flag values at the point
of execution, whether this branch is taken or not
*/
bool is_branch_taken(uint32_t opcode, uint32_t flags)
{
  ...
  ASSERT_MSG(
      (!unhandled),
      ("ERROR: is brach taken opcode %d not handled in canonicalization\n",
       opcode));
...
Parameters
 opcode - x86 instruction opcode, make sure this instruction is a conditional
jump flags	- the flags bit mask vector of the condition codes for a given
instruction, obtained through is_eflags_affected function

Return
 bool - whether the condition codes that are updated by a particular instruction
is actually consumed by the conditional jump.

 However, note that it is up to the user to perform reaching definitions
analysis to find the correct instruction which determines branching direction.
This function will only say whether given the flags mask vector for a particular
instruction, will it potentially affect the branching decision for the
...
bool is_jmp_conditional_affected(uint32_t opcode, uint32_t flags)
{
  ...
  ASSERT_MSG(
      (!unhandled),
      ("ERROR: jmp affected opcode %d not handled in canonicalization\n",
       opcode));
...
{
  ASSERT_MSG(
      (needed_src == actual_src) && (needed_dst == actual_src),
      ("ERROR: opcode %d - needed %d(src) %d(dst), actual %d(src) %d(dst)\n",
