...
vector<vector<uint32_t>> find_dependant_statements_with_indirection(
    vec_cinstr& instrs, vector<mem_regions_t*> mem,
    std::vector<Static_Info*> static_info, vector<uint32_t> start_points)
{
  ...
  for (int i = 0; i < start_points.size(); i++) {
    ...
    for (int j = 0; j < mem.size(); j++) {
      ...
      if (mem[j]->start < mem[j]->end) {
        for (uint64_t i = mem[j]->start; i < mem[j]->end;
             i += mem[j]->bytes_per_pixel) {
          operand_t opnd = {MEM_HEAP_TYPE, mem[j]->bytes_per_pixel, i};
          if (direct_tree->search_node(&opnd) == NULL) {
            direct_tree->add_to_frontier(
                direct_tree->generate_hash(&opnd), new Conc_Node(&opnd));
          }
          if (indirect_tree->search_node(&opnd) == NULL) {
            indirect_tree->add_to_frontier(
                indirect_tree->generate_hash(&opnd), new Conc_Node(&opnd));
          }
        }
      } else {
        for (uint64_t i = mem[j]->start; i >= mem[j]->end;
    ...
    for (int i = start; i < end; i++) {
      ...
      for (int j = 0; j < amount; j++) {
        if (direct_tree->update_dependancy_forward(
                &rinstr[j], instr->pc, para, i + 1)) {
          direct_dependant = true;
          value = j;
        }
        if (indirect_tree->update_dependancy_forward_with_indirection(
                &rinstr[j], instr->pc, para, i + 1)) {
      ...
      if (direct_dependant) {
        if (find(direct_app_pc.begin(), direct_app_pc.end(), instr->pc)
            == direct_app_pc.end()) {
      ...
      if (indirect_dependant) {
        if (find(indirect_app_pc.begin(), indirect_app_pc.end(), instr->pc)
            == indirect_app_pc.end()) {
...
void populate_dependant_indireciton(
    vector<Static_Info*>& static_info, vector<uint32_t> dep)
{
