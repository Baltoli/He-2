...
std::vector<uint32_t> find_dependant_statements(
    vec_cinstr& instrs, mem_regions_t* mem,
...
void update_merge_points(vec_cinstr& instrs, vector<Jump_Info*>& jumps)
{
  ...
  for (int i = 0; i < jumps.size(); i++) {
    ...
    if (jumps[i]->target_pc != 0 && jumps[i]->fall_pc != 0) {
      ...
      ASSERT_MSG(
          (jumps[i]->target_pc >= jumps[i]->fall_pc),
          ("ERROR: we only consider forward branches for now\n"));
    }
    ...
    if (true_line == 1 || false_line == 1) {
      DEBUG_PRINT(
          ("WARNING: either taken or not taken information is missing for this "
           "trace for jump %d\n",
           jumps[i]->jump_pc),
          2);
      // ASSERT_MSG(false, ("ERROR: missing a certain direction of the branch;
      // please use another representative input\n"));
      continue;
    }
...
vector<Jump_Info*> find_dependant_conditionals(
    vector<uint32_t> dep_instrs, vec_cinstr& instrs,
    ...
    if (is_conditional_jump_ins(instr->opcode)) {
      ...
      while (!found) {
        ...
        if (eflags && is_jmp_conditional_affected(instr->opcode, eflags)) {
          ...
          found = true;
          if (find(dep_instrs.begin(), dep_instrs.end(), j_instr->pc)
              != dep_instrs.end()) {
            ...
            if (!jump_found) {
              ...
              if (is_branch_taken(
                      instr->opcode, instr->eflags)) { /* jump is taken */
              ...
              if (is_branch_taken(instr->opcode, instr->eflags)) {
                ...
                if (jump_cinstr->fall_pc == 0) {
                  ...
                  if (jump_cinstr->fall_pc != instrs[i + 1].first->pc) {
                    ...
                    cout << jump_cinstr->jump_pc << " "
                         << instrs[i + 1].first->pc << " "
                         << jump_cinstr->fall_pc << " " << disasm << endl;
                  }
                  // ASSERT_MSG((jump_cinstr->fall_pc == instrs[i +
                  // 1].first->pc), ("ERROR: inconsistency fall %d\n", i + 1 ));
                }
              }
            }
          }
...
void populate_conditional_instructions(
    vector<Static_Info*>& static_info, vector<Jump_Info*> jumps)
{
  ...
  for (int i = 0; i < static_info.size(); i++) {
    ...
    for (int k = 0; k < jumps.size(); k++) {
      ...
      // sbb, adc etc.
      // cout << pc << endl;
      if ((pc == jumps[k]->jump_pc)
          && (jumps[k]->target_pc == jumps[k]->fall_pc)
          && (jumps[k]->target_pc == jumps[k]->merge_pc)) {
