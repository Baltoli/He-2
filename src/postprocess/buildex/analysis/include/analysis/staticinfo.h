...
/* static information storage for dynamically executed instructions - this data
...
struct Jump_Info {
  ...
  // example lines in the instruction trace for taken and notTaken jump
  // conditionals
  uint32_t taken;
  uint32_t not_taken;
};
...
public:
  ...
  uint32_t pc; // the program counter value for this instruction - for a jump
               // instruction this is the jump pc
  std::string disassembly; // disassembly string

  Instr_type type; // type of the instruction
  std::vector<std::pair<Jump_Info*, bool>>
      conditionals; // are there any input dependent conditionals?
