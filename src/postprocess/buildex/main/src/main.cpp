...
void print_usage()
{
  ...
  printf("\t debug_level - the level of debugging (higher means more debug "
         "info) \n");

  printf("\t seed - the seed to select the random memory point\n");
  printf("\t tree_build - the tree building method \"random - 1\",\"random set "
         "- 2\",\"similar - 3\",\"clustered - 4\"\n");
  printf("\t mode - the mode in which this tool is running mem_info_stage, "
         "tree_build_stage, abstraction_stage, halide_output_stage\n");

  printf("\t dump - whether the application memory dump should be used\n");
  printf(
      "\t start_pcs - a comma separated list of start of captured functions\n");
  printf("\t end_pcs - a comma separated list of end of captured functions\n");

  /*auxiliary variables*/
  printf("\t version - version of the instrace - with and without addr calc "
         "information\n");
  printf("\t skip - take the nth tree for abstraction\n");
  printf("\t no_trees - number of trees to be included in the abstraction "
         "process\n");
...
int main(int argc, char** argv)
{
  ...
  ASSERT_MSG(
      (start_pcs.size() == end_pcs.size()),
      ("ERROR: start and end pcs sizes should match\n"));
  ASSERT_MSG(!exec.empty(), ("exec must be specified\n"));
  ASSERT_MSG(
      (!in_image.empty()) && (!out_image.empty()),
  ...
  if (thread_id != -1) { /* here we can get a specific instrace file*/
    instrace_filename = get_standard_folder("output") + "\\instrace_" + exec
    ...
    ASSERT_MSG(
        (!instrace_filename.empty()), ("suitable instrace file cannot be "
                                       "located; please specify manually\n"));
    instrace_file.open(instrace_filename, ifstream::in);
  }
  ...
  ASSERT_MSG(
      (!disasm_filename.empty()), ("suitable disasm file cannot be located\n"));
  ...
  /* get the image mem config files - these have common configs for a given
   * image processing program like Photoshop (hardcoded)  */
  ...
  if (debug) {
    /* get the log file */
    log_file.open(
        get_standard_folder("log") + file_substr + ".log", ofstream::out);
  }

  halide_file.open(
      get_standard_folder("halide") + file_substr + "_halide.cpp",
      ofstream::out);
  ...
  */
   ...
   * this is the case for non-image processing programs.
   */
  vector<mem_regions_t*> dump_regions;
  if (dump) {
    dump_regions = get_image_regions_from_dump(
        memdump_files, in_image_filename, out_image_filename);
  ...
  /* problems pc mem extraction and mem region is not getting the same results?
   * why? */
  ...
  vector<vector<mem_info_t*>> mergable
      = get_merge_opportunities(mem_info, pc_mem_info);
  merge_mem_regions_pc(mergable, mem_info);

  /* print based on levels */
  for (int i = 0; i < mem_info.size(); i++) {
    LOG(log_file,
        "start main " << mem_info[i]->start << " " << mem_info[i]->end << endl);
    mem_info_t* mems = mem_info[i];
    while (mems->mem_infos.size() > 0) {
      LOG(log_file, "stage");
      for (int j = 0; j < mems->mem_infos.size(); j++) {
        LOG(log_file, mems->mem_infos[j]->start
                          << " " << mems->mem_infos[j]->end << endl);
      }
  ...
  DEBUG_PRINT(
      ("*******************end of mem info stage*********************\n"), 2);

  vector<mem_info_t*> mems_temp = mem_info;
  sort(
      mems_temp.begin(), mems_temp.end(),
      [](mem_info_t* first, mem_info_t* second) -> bool {
  ...
  DEBUG_PRINT(
      ("*******************instruction gathering/preprocessing "
       "stage*********************\n"),
      2);
  ...
  vec_cinstr instrs_forward_unfiltered
      = walk_file_and_get_instructions(instrace_file, static_info, version);
  /* need to filter unwanted instrs from the file we got */
  vec_cinstr instrs_forward
      = filter_instr_trace(start_pcs, end_pcs, instrs_forward_unfiltered);
  ...
  for (int i = instrs_forward.size() - 1; i >= 0; i--) {
    ...
    pair<cinstr_t*, Static_Info*> instr_string
        = make_pair(new_cinstr, instrs_forward[i].second);
  ...
  update_floating_point_regs(
      instrs_backward, BACKWARD_ANALYSIS, static_info, start_pcs);
  update_floating_point_regs(
      instrs_forward, FORWARD_ANALYSIS, static_info, start_pcs);

  DEBUG_PRINT(
      ("*******************end of instruction gathering/preprocessing "
       "stage*********************\n"),
      2);

  /*******************************************more memory and input/output
   * selection********************************************************/
  ...
  remove_possible_stack_frames(
      pc_mem_info, mem_info, static_info, instrs_forward);

  /* merge these two information - instrace mem info + mem dump info */

  vector<mem_regions_t*> total_mem_regions;
  vector<mem_regions_t*> image_regions = merge_instrace_and_dump_regions(
      total_mem_regions, mem_info, dump_regions);

  /* get possible buffers - should be able to merge these analysis?? */
  mark_possible_buffers(
      pc_mem_info, total_mem_regions, static_info, instrs_forward);
  vector<mem_regions_t*> regions = get_input_output_regions(
      image_regions, total_mem_regions, pc_mem_info, candidate_ins,
      instrs_forward, start_points_mem);
  ...
  if ((anaopt & INPUT_REGION_SELECTION) == INPUT_REGION_SELECTION) {
    ...
    input_regions = get_input_regions(
        total_mem_regions, pc_mem_info, start_points_mem, instrs_forward);
  ...
  image_regions.clear();
  image_regions.push_back(
      output_mem_region); /* we do not need to check whether this region is
                             there; can be a duplicate, this is for random tree
                             building */
  ...
  DEBUG_PRINT(
      ("******************memory input and output selection "
       "done********************************\n"),
      2);
  ...
  if ((anaopt & DEPENDANT_ANALYSIS) == DEPENDANT_ANALYSIS) {
    ...
    app_pc_vec = find_dependant_statements_with_indirection(
        instrs_forward, input_regions, static_info, start_points_mem);
  } else {
  ...
  it = set_difference(
      app_pc_vec[1].begin(), app_pc_vec[1].end(), app_pc_vec[0].begin(),
      app_pc_vec[0].end(), app_pc_indirect.begin());
  ...
  if ((anaopt & CONDITIONAL_ANALYSIS) == CONDITIONAL_ANALYSIS) {
    cond_app_pc = find_dependant_conditionals(
        app_pc_total, instrs_forward, static_info);
  }

  LOG(log_file, "dependant conditionals" << endl);
  for (int i = 0; i < cond_app_pc.size(); i++) {
    string dis_jmp_string
        = get_disasm_string(static_info, cond_app_pc[i]->jump_pc);
    string dis_cond_string
        = get_disasm_string(static_info, cond_app_pc[i]->cond_pc);
  ...
  for (int i = 0; i < static_info.size(); i++) {
    ...
    if (info->conditionals.size() > 0) {
      ...
      for (int j = 0; j < info->conditionals.size(); j++) {
        LOG(log_file, "cond " << info->conditionals[j].first->cond_pc << "jump "
                              << info->conditionals[j].first->jump_pc
                              << " taken : " << info->conditionals[j].second
                              << endl);
      }
    }
  }
  ...
  /* capture the function start points if the end trace is not given
   * specifically */
  ...
  if (tree_build == BUILD_RANDOM) {
    ...
    if (start_trace == FILE_BEGINNING) {
      mem_regions_t* random_mem_region
          = get_random_output_region(image_regions);
      ...
      ASSERT_MSG(
          (stride != 0 && dest != 0), ("ERROR: if the starting point is given "
                                       "please specify the dest and stride\n"));
    }
    DEBUG_PRINT(("func pc entry - %x\n", start_pc), 1);

    // Node * node = create_tree_for_dest(dest, stride, instrace_file,
    // start_points, start_trace, end_trace, disasm)->get_head();
    Conc_Tree* tree = new Conc_Tree();
    Conc_Tree* initial = build_conc_tree(
        dest, stride, start_points, start_trace, end_trace, tree,
        instrs_backward, farthest, total_mem_regions, func_replacements);
    tree->print_conditionals();
    DEBUG_PRINT(("creating conditional trees\n"), 2);
    build_conc_trees_for_conditionals(
        start_points, tree, instrs_backward, farthest, total_mem_regions,
        func_replacements);
    ...
    if (initial != NULL) {
      build_conc_trees_for_conditionals(
          start_points, initial, instrs_backward, farthest, total_mem_regions,
          func_replacements);
    ...
    vector<uint64_t> nbd_locations
        = get_nbd_of_random_points(image_regions, seed, &stride);
    ...
    for (int i = 0; i < nbd_locations.size(); i++) {
      ...
      Conc_Tree* tree = new Conc_Tree();
      Conc_Tree* initial = build_conc_tree(
          nbd_locations[i], stride, start_points, FILE_BEGINNING, end_trace,
          tree, instrs_backward, farthest, total_mem_regions,
          func_replacements);
      build_conc_trees_for_conditionals(
          start_points, tree, instrs_backward, farthest, total_mem_regions,
          func_replacements);
    ...
    vector<vector<Conc_Tree*>> clustered_conc_trees
        = categorize_trees(conc_trees);
    ASSERT_MSG(
        (clustered_conc_trees.size() == 1),
        ("ERROR: The trees should be similar\n"));

  } else if (tree_build == BUILD_SIMILAR) {
    uint64_t farthest = get_farthest_mem_access_point(total_mem_regions);
    conc_trees = get_similar_trees(
        image_regions, total_mem_regions, seed, &stride, start_points,
        start_trace, end_trace, farthest, instrs_backward, func_replacements);
  } else if (tree_build == BUILD_CLUSTERS) {
    uint64_t farthest = get_farthest_mem_access_point(total_mem_regions);
    clustered_trees = cluster_trees(
        image_regions, total_mem_regions, start_points, instrs_backward,
        farthest, output_folder + file_substr, func_replacements);
  }
  ...
  if (clustered_trees.size() > 0) {
    ...
    for (int i = 0; i < conc_trees.size(); i++) {
      ...
      ofstream expression_file(
          output_folder + file_substr + "_expression_" + to_string(i) + ".txt",
          ofstream::out);
  ...
  if (clustered_trees.size() > 0) {
    abs_trees = build_abs_trees(
        clustered_trees, output_folder, no_trees, total_mem_regions, skip,
        pc_mem_info);
    ...
    for (int i = 0; i < abs_trees.size(); i++) {
      ofstream abs_file(
          output_folder + file_substr + "_abstree_" + to_string(i) + ".dot",
          ofstream::out);
    ...
    ofstream comp_file(
        output_folder + file_substr + "_comp_tree.dot", ofstream::out);
    comp_tree->print_dot(comp_file, "comp", 0);
    comp_tree->abstract_buffer_indexes();
    final_abs_tree = comp_tree->compound_to_abs_tree();

    ofstream alg_file(
        output_folder + file_substr + "_algebraic.dot", ofstream::out);
    uint32_t max_dimensions = final_abs_tree->get_maximum_dimensions();
    final_abs_tree->print_dot_algebraic(
        alg_file, "alg", 0, get_vars("x", max_dimensions));
  }
  ...
  if (abs_trees.size() == 0) {
    ...
    for (int i = 0; i < abs_trees.size(); i++) {
      if (abs_trees[i]->is_recursive) {
        ...
        halide->populate_red_funcs(
            abs_trees[i]->tree, abs_trees[i]->extents, abs_trees[i]->red_node);
      } else {
  ...
  /* dumping memory values to files for debugging lifted halide filters - should
   * be done separately */
