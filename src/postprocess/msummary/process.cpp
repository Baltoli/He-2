...
int main()
{
...
#else
  trace = fopen(
      "C:\\Charith\\Dropbox\\Research\\development\\memtrace\\build_"
      "32\\bin\\memtrace.4588.log",
      "r");
#endif
  ...
  while (fscanf(
             trace, "%x,%c,%u,%x\n", &instruction_addr, &read_write,
             &access_width, &mem_addr)
         == 4) {
    ...
    for (int i = 0; i < per_ins_accesses[index].size(); i++) {
      ...
      if (mem_addr + access_width > range.second
          && mem_addr <= range.second) { // [ *******-------]---- can be the
                                         // case that mem_addr >= range.first
      ...
      if (mem_addr + access_width >= range.first
          && mem_addr
                 < range.first) { // ----[-----******]  can be the case that
                                  // mem_addr + access_width > range.second
        per_ins_accesses[index][i].first = mem_addr;
        if (mem_addr + access_width
            > range.second) { // this is actually covered by the condition in
                              // the previous if
          per_ins_accesses[index][i].second = mem_addr + access_width;
        }
        status = 1;
    ...
    // assume that the memory is accessed in a manner that is strided; no
    // irregular accesses
    if (status == 0)
      per_ins_accesses[index].push_back(
          make_pair(mem_addr, mem_addr + access_width));
  }
